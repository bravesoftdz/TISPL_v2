...
void __fastcall TForm1::FormKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
{
	...
	switch(Key)
	{
		...
		case 27:
		{
          ...
          case -13:if(pom_element_temp->geo.HeightDepp>0)editovany_text="+";else editovany_text="";editovany_text+=pom_element_temp->geo.HeightDepp*1000;break;//editace HeightDepp S/K elementu
				}
    		if(index_kurzoru==-9||index_kurzoru==-8)//editace kót kabiny
    		{
				...
				if(index_kurzoru==-11 && (OBJEKT_akt->id!=3 || (OBJEKT_akt->id==3 && (Akce==GEOMETRIE || Akce==GEOMETRIE_LIGHT))))//editace kót elementů
    		{
		...
		//-
		case 109:if(!editace_textu)ZOOM_OUT();break;
		//F1 - volání nápovědy
		case 112:break;
		//F2
...
void __fastcall TForm1::FormKeyPress(TObject *Sender, System::WideChar &Key)
{
	...
	if(!((Key>=L'0')&&(Key<=L'9') || String(Key)==ms.get_locale_decimal() || (index_kurzoru==-13 && editovany_text.Length()==0 && (String(Key)=="+" || String(Key)=="-"))))
		key="";
	...
	if (editace_textu&&(index_kurzoru==-13||index_kurzoru==-11||index_kurzoru==-2||index_kurzoru==-5))//editace HeightDepp S/K elementu,kót elementů,kót haly nebo objektu, kót kabiny
	{
		if(Key==8)//pokud je stisknut backspace
			...
			if(Button==mbLeft)//je stisknuto levé tlačítko myši
			{
				...
				if(Akce==NIC && posun_objektu==false && funkcni_klavesa==0)//pokud není aktivovaná jiná akce
				{
					if(MOD==EDITACE && OBJEKT_akt!=NULL)//TABULKA či ELEMENT
					{
						...
						if(JID==-13){nastav_focus();TimerKurzor->Enabled=true;editace_textu=true;stav_kurzoru=false;index_kurzoru=JID;pom_element_temp=pom_element;if(pom_element->geo.HeightDepp>0)editovany_text="+";else editovany_text="";editovany_text+=pom_element->geo.HeightDepp*1000;}//editace HeightDepp S/K elementu
	...
	if(Button==mbLeft)//zohlední jenom stisk levého tlačítka myši
	{
        ...
        //vytvoření obrazu pro UNDO a REDO
			...
			case MOVE_ELEMENT:
			{
				...
				if(OBJEKT_akt!=NULL && vychozi_souradnice_kurzoru.x==minule_souradnice_kurzoru.x && vychozi_souradnice_kurzoru.y==minule_souradnice_kurzoru.y){nastav_focus();stav_kurzoru=false;index_kurzoru=JID;pom_element_temp=pom_element;nazev_puvodni=pom_element_temp->name;editace_textu=true;TimerKurzor->Enabled=true;}
				else vytvor_obraz();
			...
			case MOVE_TEXT:if(OBJEKT_akt!=NULL && vychozi_souradnice_kurzoru.x==minule_souradnice_kurzoru.x && vychozi_souradnice_kurzoru.y==minule_souradnice_kurzoru.y){nastav_focus();editace_textu=true;index_kurzoru=-6;nazev_puvodni=OBJEKT_akt->name;stav_kurzoru=false;TimerKurzor->Enabled=true;}Akce=NIC;kurzor(standard);pom_vyhybka=NULL;vytvor_obraz();break;
			case MOVE_BOD:
			...
			if(JID==-1)
			{
				...
				while(pom_element!=NULL)
				{
					if(pom_element->geo.HeightDepp!=0)//pouye pro elementy s S/K
					{
				  	...
				  	{
				  		oblast.top=pom_element->citelna_oblast.rect6.top;
           		oblast.bottom=pom_element->citelna_oblast.rect6.bottom;
				  	}
						else
						{
						...
						if(pom_element->geo.HeightDepp<0)oblast_pom=pom_element->citelna_oblast.rect6;
						...
						//hledání, zda jsem v oblasti, nelze použít PtInRect, vracel nekorektní výsledky pro určité orientace
	...
	if(MOD==EDITACE)
	{
		...
		if(puvJID!=JID || pom_element_puv!=pom_element || pom_komora_puv!=pom_komora)//pokud došlo ke změně JID, nebo změně elementu bez změny JID (např. situace dva roboti vedle sebe nebo rychlý přesun), jinak nemá smysl řešit
		{
  		...
  		if(JID==1000){pom_element->mGrid->CheckLink(X,Y);refresh_mGrid=true;}//pouze pro přechod název hlavička, aby název nezůstal tučně - aktivace dodáním pouze aktuálních souřadnic
			...
			if((JID==-11 || JID==-13)&&!editace_textu){kurzor(edit_text);refresh_mGrid=false;}//kurzor pro editaci textu, kóty + S/K
...
void TForm1::onPopUP(int X, int Y)
{
	...
	switch(MOD)
	{
		...
		case EDITACE:
		{
			...
			if(pom_vyhybka!=NULL && pom_vyhybka!=OBJEKT_akt){PopUPmenu->Item_nastavit_parametry->Visible=true;PopUPmenu->Panel_UP->Height+=34;}
      ...
      ////////nastavení horního panelu
			...
			//smazání elementu
			if(pom_element!=NULL && (pom_element->eID!=200 || (pom_element->eID==200 && ((pom_element->dalsi!=NULL && pom_element->dalsi->pohon==pom_element->pohon) || (pom_element->dalsi==NULL && d.v.ELEMENTY->dalsi->pohon==pom_element->pohon)))))//Pokud bylo kliknuto na element + ošetření pom_element je používan i při tvorbě geometrie + nejedná se o předávací místo
			{
        ...
        //mazání S/K
				if(pom_element->geo.HeightDepp>0 && (JID==-12 || JID==-13))PopUPmenu->scLabel_smazat->Caption=smazat+" "+ls->Strings[490];
				if(pom_element->geo.HeightDepp<0 && (JID==-12 || JID==-13))PopUPmenu->scLabel_smazat->Caption=smazat+" "+ls->Strings[491];
	...
	if(bod_vlozeni.x!=-1000 && pom_element_temp!=NULL)//kontrola zda vkládám na přímku
	{
      ...
      if(E->eID!=5 && E->eID!=6 && E->eID!=100 && E->eID!=200)d.v.aktualizuj_rezim_pohonu(E->pohon,d.v.vrat_druh_elementu(E));//přeskakovat elementy co neudávají režim
    	...
    	//bylo vloženo předávací místo
   		if(E->eID==200)
   		{          ///zakomentovaná konstukce slouží k nastavení pohonu za vkládáným PM do dalšího PM na NULL
	...
	while(E!=NULL)
	{
		...
		if(E->eID!=300 && E->eID!=301 && !(E->dalsi!=NULL && E->dalsi->eID==301 && E->dalsi->predchozi2==E))//výhybky a spojky jsou "PM"
		{
          ...
          el->pohon=E->pohon;
					...
					if(el->dalsi!=NULL && el->dalsi->pohon!=NULL)el->WT=m.cekani_na_palec(0,el->dalsi->pohon->roztec,el->dalsi->pohon->aRD,3);
					//souřadnice tabulky
...
//						}
	  	...
	  	//////////Aktualizace WT
			if(E->eID==200)
			{
				...
				{
					if(E->WT!=0)E->mGrid->Cells[sloupec][11].Text=F->m.round2double(F->outPT(E->WT),3);
					else E->mGrid->Cells[sloupec][11].Text="-";
				}
			}
		}
...
//					}
		...
		if(e_posledni->eID==200)
		{
			...
			if(OBJEKT_akt!=NULL && OBJEKT_akt->n==e_posledni->objekt_n)
			{
				if(E->WT!=0)E->mGrid->Cells[sloupec][11].Text=F->m.round2double(F->outPT(E->WT),3);
				else E->mGrid->Cells[sloupec][11].Text="-";
			}
...
void TForm1::ukonceni_geometrie(bool kontrola)
{
  ...
  ////aktualizace popisků
	...
	bool zmena=false;
	if(kontrola)zmena=pripnuti_dalsich_objektu();

	//pokud jsem v prvním nebo posledním objektu kontrola zda je linka spojená, nebo pokud dojde k pripnutí dalších objektů
	if(kontrola && (OBJEKT_akt->n==1 || OBJEKT_akt->n==d.v.OBJEKTY->predchozi->n || zmena))spojeni_prvni_posledni();//kontrola zda mám dostatečný počet objektů a zda je možno spojit je automaticky
	...
	else if(E->pohon==OBJEKT_akt->pohon)////nastavení komponent, pouze pokud jsou tabulky viditelné
	{
        ...
        ////temp odemknutí editace max WT
  		...
  		case 3://robot s pasivní otočí
  		{
				...
				E->mGrid->Cells[3][3].Type=E->mGrid->COMBO;
				E->mGrid->Cells[3][5].Type=E->mGrid->EDIT;
				E->mGrid->Cells[3][8].Type=E->mGrid->EDIT;
				E->mGrid->Cells[3][9].Type=E->mGrid->EDIT;
  			E->mGrid->Cells[3][10].Type=E->mGrid->COMBO;
				E->mGrid->exBUTTONVisible=true;
				E->mGrid->Update();
  			break;
  		}
	...
	if(data && PmG!=NULL)
	{
  	if(OBJEKT_akt->pohon!=NULL)
  	{
  		...
  		PmG->Cells[3][2].Text=AnsiString(m.round2double(outaRD(OBJEKT_akt->pohon->rychlost_od),3))+" - "+AnsiString(m.round2double(outaRD(OBJEKT_akt->pohon->rychlost_do),3));
			...
			if(OBJEKT_akt->pohon->rezim!=0)PmG->Cells[3][5].Text=m.round2double(OBJEKT_akt->pohon->Rx,3);
			if(OBJEKT_akt->pohon->rezim!=0)PmG->Cells[3][3].Text=m.round2double(outR(OBJEKT_akt->pohon->Rz),3);
			if(OBJEKT_akt->pohon->rezim!=0)PmG->Cells[3][6].Text=m.round2double(outRz(m.mezera(0,OBJEKT_akt->pohon->Rz,0)),3);
			if(OBJEKT_akt->pohon->rezim!=0)PmG->Cells[3][7].Text=m.round2double(outRz(m.mezera(0,OBJEKT_akt->pohon->Rz,1)),3);
			if(OBJEKT_akt->pohon->rezim!=0)PmG->Cells[3][8].Text=m.round2double(outRz(m.mezera(90,OBJEKT_akt->pohon->Rz,1)),3);
  	}
  	else
  	{
			PmG->Cells[3][1].Text="-";
...
void TForm1::napln_comba_mGridu(Cvektory::TElement *E)
{
  ...
  FormX->vstoupeno_elm=false;
	...
	if(E->eID==3||E->eID==4||E->eID==5||E->eID==6||E->eID==9||E->eID==10||E->eID==13||E->eID==14||E->eID==17||E->eID==18||E->eID==103||E->eID==104||E->eID==107||E->eID==108)//elementy s otočí
	{
		...
		{
			case 3:case 9:case 13:case 17:case 103:case 107:sloupec=3;pozice=3;break;
...
void TForm1::zmena_editovanych_bunek(Cvektory::TElement *E)
{
	...
	if(E->eID==200 || E->eID==300)//pouze pro pohonové tabulky
	{
		...
		{
			if(E->mGrid->Cells[prvni][3].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,prvni,3);if(E->mGrid->Note.Text==""){E->mGrid->Cells[prvni][3].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[prvni][3].Font->Color=(TColor)RGB(128,128,128);}}
			if(E->mGrid->Cells[prvni][6].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,prvni,6);if(E->mGrid->Note.Text==""){E->mGrid->Cells[prvni][6].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[prvni][6].Font->Color=(TColor)RGB(128,128,128);}}
			if(E->mGrid->Cells[prvni][7].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,prvni,7);if(E->mGrid->Note.Text==""){E->mGrid->Cells[prvni][7].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[prvni][7].Font->Color=(TColor)RGB(128,128,128);}}
		}
		...
		if(C1!=NULL && C1->Enabled)
		{
	  	...
	  	if(E->pohon!=NULL && !d.v.pohon_je_pouzivan(E->pohon->n,false))
	  	{
	  		...
	  		{
					E->mGrid->Cells[prvni][3].Type=E->mGrid->EDIT;if(E->mGrid->Note.Text==""){E->mGrid->Cells[prvni][3].Background->Color=clWhite;E->mGrid->Cells[prvni][3].Font->Color=(TColor)RGB(43,87,154);}
					E->mGrid->Cells[prvni][6].Type=E->mGrid->EDIT;if(E->mGrid->Note.Text==""){E->mGrid->Cells[prvni][6].Background->Color=clWhite;E->mGrid->Cells[prvni][6].Font->Color=(TColor)RGB(43,87,154);}
					E->mGrid->Cells[prvni][7].Type=E->mGrid->EDIT;if(E->mGrid->Note.Text==""){E->mGrid->Cells[prvni][7].Background->Color=clWhite;E->mGrid->Cells[prvni][7].Font->Color=(TColor)RGB(43,87,154);}
	  		}
	  		else//povolit editaci RD a rozteče palce
	  		{
					E->mGrid->Cells[prvni][3].Type=E->mGrid->EDIT;if(E->mGrid->Note.Text==""){E->mGrid->Cells[prvni][3].Background->Color=clWhite;E->mGrid->Cells[prvni][3].Font->Color=(TColor)RGB(43,87,154);}
	...
	switch(E->eID)
	{
      ...
      ////konec temp
		...
		case 1://robot (kontinuální)
		{
			...
			E->mGrid->Cells[0][2].ShowHint=true;
			E->mGrid->Cells[0][3].Hint=t_hint2;
			E->mGrid->Cells[0][3].ShowHint=true;
      ...
      //pokud je záporné RT, změn PT
		...
		case 3://robot s pasivní otočí
		{
			...
			E->mGrid->Cells[3][1].Type=E->mGrid->EDIT;E->mGrid->Cells[3][1].Text=outPT(m.PT(E->data.LO1,aRD));
			E->data.PT1=inPT(ms.MyToDouble(E->mGrid->Cells[3][1].Text));
			E->mGrid->Cells[0][2].Text=t+"1 "+LO;
			E->mGrid->Cells[3][2].Type=E->mGrid->EDIT;E->mGrid->Cells[3][2].Text=outLO(E->data.LO1);
			E->mGrid->Cells[0][2].BottomBorder->Width=2;
			E->mGrid->Cells[3][2].BottomBorder->Width=2;
			E->mGrid->Cells[0][3].Font->Orientation=900;E->mGrid->Cells[0][3].Valign=TmGrid::MIDDLE;
			E->mGrid->Cells[0][3].Text=ls->Strings[240];//"Otoč"
			E->mGrid->Cells[1][3].Text=ls->Strings[239]+" [°]";//"rotace [°]";
			E->mGrid->Cells[3][3].Type=E->mGrid->COMBO;
			E->mGrid->Cells[1][4].Text=ls->Strings[241]+" "+cas;//"Otáčení"
			E->mGrid->Cells[3][4].Text=outPT(m.PTo(E->OTOC_delka,aRD));
			E->PTotoc=inPT(ms.MyToDouble(E->mGrid->Cells[3][4].Text));
			E->mGrid->Cells[1][5].Text=ls->Strings[246]+" "+delka_otoce;//"délka "
			E->mGrid->Cells[3][5].Type=E->mGrid->EDIT;E->mGrid->Cells[3][5].Text=outDO(E->OTOC_delka);
			E->mGrid->Cells[1][6].Text=ls->Strings[463];E->mGrid->Cells[1][6].BottomBorder->Color=clWhite;
			E->mGrid->Cells[1][7].Text=delka_otoce;
			E->mGrid->Cells[2][6].Text=ls->Strings[275];//"Zóna před "
			E->mGrid->Cells[3][6].Text=outDO(m.round2double(zony_otaceni.x,3));
			E->zona_pred=zony_otaceni.x;
			E->mGrid->Cells[2][7].Text=ls->Strings[276];//"Zóna za "
			...
			E->mGrid->Cells[3][7].BottomBorder->Width=2;
			E->mGrid->Cells[0][8].Text="PT2 "+cas;
			E->mGrid->Cells[3][8].Type=E->mGrid->EDIT;E->mGrid->Cells[3][8].Text=outPT(m.PT(E->data.LO2,aRD));
			E->data.PT2=inPT(ms.MyToDouble(E->mGrid->Cells[3][7].Text));
			E->mGrid->Cells[0][9].Text=t+"2 "+LO;
			E->mGrid->Cells[3][9].Type=E->mGrid->EDIT;E->mGrid->Cells[3][9].Text=outLO(E->data.LO2);
			E->mGrid->Cells[0][9].BottomBorder->Width=2;
			E->mGrid->Cells[3][9].BottomBorder->Width=2;
			...
			E->mGrid->Cells[0][2].Hint=t_hint;
			E->mGrid->Cells[0][2].ShowHint=true;
			E->mGrid->Cells[1][4].Hint=ls->Strings[242];//"celkový čas procesu otoče";
			E->mGrid->Cells[1][4].ShowHint=true;
			E->mGrid->Cells[1][5].Hint=ls->Strings[243];//"délka otoče";
			E->mGrid->Cells[1][5].ShowHint=true;
			E->mGrid->Cells[2][6].Hint=ls->Strings[277];//"Zóna otáčení před středem";
			E->mGrid->Cells[2][6].ShowHint=true;
			E->mGrid->Cells[2][7].Hint=ls->Strings[278];//"Zóna otáčení za středem";
...
void TForm1::dalsi_vytvoreni_tab_elementu (Cvektory::TElement *E,short sirka_0,short sirka_1,short sirka_2,short sirka_3,short sirka_4,short sirka_56,short sirka_cisla,AnsiString LO,AnsiString cas,AnsiString delka_otoce,AnsiString rychlost,AnsiString R,AnsiString Rz)
{
	...
	switch(E->eID)
	{
      ...
      bool rozbalena=true;
			...
			E->mGrid->Cells[0][10].Text="PD jig";
			E->mGrid->Cells[3][10].Type=E->mGrid->COMBO;
			E->mGrid->Cells[3][1].Text=outPT(E->data.PT1);
			E->mGrid->Cells[3][2].Text=outLO(E->data.LO1);
			E->mGrid->Cells[3][4].Text=outPT(E->PTotoc);
			E->mGrid->Cells[3][5].Text=outDO(E->OTOC_delka);
			E->mGrid->Cells[3][6].Text=outDO(m.round2double(E->zona_pred,3));
			E->mGrid->Cells[3][7].Text=outDO(m.round2double(E->zona_za,3));
			E->mGrid->Cells[3][8].Text=outPT(E->data.PT2);
			E->mGrid->Cells[3][9].Text=outLO(E->data.LO2);
			//automatické nastavení sířky sloupců podle použitých jednotek
			E->mGrid->SetColumnAutoFit(-4);
			E->mGrid->Columns[0].Width=E->mGrid->Rows[0].Height;
			E->mGrid->Columns[1].Width=2*E->mGrid->Rows[0].Height;
			...
			{
				E->mGrid->MergeCells(0,3,0,5);
				E->mGrid->VisibleRow(6,false,false);
				E->mGrid->VisibleRow(7,false,false);
				E->mGrid->exBUTTON->GlyphOptions->Kind=scgpbgkDownArrow;
			}
			else E->mGrid->MergeCells(0,3,0,7);
			E->mGrid->Cells[0][7].BottomBorder->Width=2;
			break;
		}
		case 10:case 14:case 18:case 104:case 108:
...
void TForm1::akt_tabulek (Cvektory::TElement *E,AnsiString LO,AnsiString delka_otoce,AnsiString cas,AnsiString rychlost,AnsiString R,AnsiString Rz,short sirka_0,short sirka_1,short sirka_2,short sirka_3,short sirka_4,short sirka_56,short sirka_cisla)
{
	...
	switch(E->eID)
	{
		...
		case 3:case 9:case 13:case 17:case 103:case 107://robot s pasivní otočí
		{
			...
			E->mGrid->Cells[2][1].Text="PT1 "+cas;
			E->mGrid->Cells[2][2].Text=t+"1 "+LO;
			E->mGrid->Cells[0][7].Text=ls->Strings[240];//"Otoč"
			E->mGrid->Cells[2][3].Text=ls->Strings[239]+" [°]";//"rotace [°]";
			E->mGrid->Cells[2][4].Text=ls->Strings[241]+" "+cas;//"Otáčení"
			E->mGrid->Cells[2][5].Text=ls->Strings[246]+" "+delka_otoce;//"délka "
			E->mGrid->Cells[1][6].Text=ls->Strings[463];E->mGrid->Cells[1][7].Text=delka_otoce;
			E->mGrid->Cells[2][6].Text=ls->Strings[275];//"Zóna před "
			E->mGrid->Cells[2][7].Text=ls->Strings[276];//"Zóna za "
			E->mGrid->Cells[2][8].Text="PT2 "+cas;
			E->mGrid->Cells[2][9].Text=t+"2 "+LO;
			E->mGrid->Cells[2][10].Text="PD jig";
			E->mGrid->Cells[3][1].Text=m.round2double(outPT(E->data.PT1),3);
			E->mGrid->Cells[3][2].Text=m.round2double(outLO(E->data.LO1),3);
			E->mGrid->Cells[3][4].Text=m.round2double(outPT(E->PTotoc),3);
			E->mGrid->Cells[3][5].Text=m.round2double(outDO(E->OTOC_delka),3);
			E->mGrid->Cells[3][6].Text=m.round2double(outDO(E->zona_pred),3);
			E->mGrid->Cells[3][7].Text=m.round2double(outDO(E->zona_za),3);
			E->mGrid->Cells[3][8].Text=m.round2double(outPT(E->data.PT2),3);
			E->mGrid->Cells[3][9].Text=m.round2double(outLO(E->data.LO2),3);
			E->mGrid->Columns[2].Width=sirka_3;
			break;
		}
		...
		switch(OBJEKT_akt->id)
		{
			...
			case 0:case 9://objekt navěšování + svěšování
			{
				...
				if(OBJEKT_akt->pohon!=NULL && (d.v.ZAKAZKA_akt==NULL || d.v.ZAKAZKA_akt!=NULL && d.v.ZAKAZKA_akt->n==0))
				{
			  	...
			  	else if (OBJEKT_akt->pohon->rezim==0)
			  	{
			...
			case 4://objekt ionizace
			{
				...
				if((OBJEKT_akt->pohon!=NULL && OBJEKT_akt->pohon->rezim==-1) || OBJEKT_akt->pohon==NULL)
				{
			  	for(unsigned short n=n_od;n<=n_do;n++)
			  	{
			  		//nastavení názvů
		...
		case EDITACE:       //kvůli MB hází při mazání z popUP paměťovou chybu
		{
			if(Akce!=GEOMETRIE && pom_element!=NULL && pom_element->eID!=MaxInt)//případ mazání běžného elementu
			{
				...
				if((pom_element->eID!=200 && mrYes==MB(akt_souradnice_kurzoru_PX.x+10,akt_souradnice_kurzoru_PX.y+10,text+pom_element_temp->name.UpperCase()+"?","",MB_YESNO)))// || (pom_element->eID==200 && pom_element->dalsi!=NULL && pom_element->objekt_n==OBJEKT_akt->n && pom_element->dalsi->objekt_n==OBJEKT_akt->n) && mrYes==MB(akt_souradnice_kurzoru_PX.x+10,akt_souradnice_kurzoru_PX.y+10,text_PM,"",MB_YESNO))
				{
					...
					if(pom_element_temp->dalsi!=NULL && p!=NULL && p->rezim==0)dalsi_element=pom_element_temp->dalsi;//pokud je kabina ve S&G režimu a za smazaným elementem se nachází další element, je nutné dalšímu přepočítat RT
...
//	Memo("delkaPud: "+String(E->geo.delkaPud));
	//deklarace
//	Cvektory::TObjekt *Objekt=d.v.PtInObjekt();
//	TPointD bod_vlozeni=bod_vlozeni_elementu();
//	Cvektory::TElement *novy=new Cvektory::TElement;
//
//	//parametry
//	novy->eID=MaxInt;
//  novy->idetifikator_vyhybka_spojka=0;
//	novy->X=bod_vlozeni.x;
//	novy->Y=bod_vlozeni.y;
//  novy->Z=0;
//	novy->Xt=-100;
//	novy->Yt=-100;
//	novy->orientace=90;//důležité pro volání makra m.Rxy, bez tohoto by makro vracelo chybné hodnoty
//	novy->dalsi=NULL;
//	novy->predchozi=NULL;
//	novy->dalsi2=NULL;
//	novy->predchozi2=NULL;
//	//řazení
//	novy->dalsi=pom_element_temp;
//	pom_element_temp->predchozi->dalsi=novy;
//	novy->predchozi=pom_element_temp->predchozi;
//	pom_element_temp->predchozi=novy;
//	//defaultní data
//  novy->name="Zarážka";
//	novy->data.LO1=1.5;
//	novy->OTOC_delka=0;
...
//  novy->sparovany=NULL;
//	if(Objekt->element==novy->dalsi)Objekt->element=novy;
...
//
//	//geometrie
//	d.v.vloz_G_element(novy,0,pom_element_temp->geo.X1,pom_element_temp->geo.Y1,0,0,0,0,bod_vlozeni.x,bod_vlozeni.y,pom_element_temp->geo.orientace,pom_element_temp->geo.rotacni_uhel,pom_element_temp->geo.radius);
//	d.v.vloz_G_element(pom_element_temp,0,novy->geo.X4,novy->geo.Y4,0,0,0,0,pom_element_temp->geo.X4,pom_element_temp->geo.Y4,pom_element_temp->geo.orientace,pom_element_temp->geo.rotacni_uhel,pom_element_temp->geo.radius);
//
//  //update n a popisků
//	novy=d.v.ELEMENTY->dalsi;
//	unsigned int n=1;
//	while(novy!=NULL)
//	{
//		novy->n=n;
//		n++;
//		novy=d.v.dalsi_krok(novy);
//	}
//  d.v.uprav_popisky_elementu(NULL);
//	novy=NULL;delete novy;
//	Objekt=NULL;delete Objekt;
	Memo(d.v.ELEMENTY->dalsi->dalsi->dalsi->name+"->pocet_voziku: "+String(d.v.ELEMENTY->dalsi->dalsi->dalsi->data.pocet_voziku));
	Memo(d.v.vrat_druh_elementu(d.v.ELEMENTY->dalsi->dalsi->dalsi));
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
void TForm1::vykresli_kurzor(int index)
{
  log(__func__);//logování
	...
	switch ((index))//index=JID, kde a jaký kurzor vykreslit
	{
		...
		case -11://kóty elementů, kóty komor
		{
...
void TForm1::smaz_kurzor()
{
  log(__func__);//logování
    ...
    //pokud je kurzor vykreslen překresli ho
      ...
      editace_textu=false;//důležité, pokud se zobrazí MB s překrytím zobrazí se editovaný text, v němž je posun a ne hodnota délky v mm
    ...
    //editace rozměrů přímky v editaci geometrie
		if(index_kurzoru==-11&&(Akce==GEOMETRIE||Akce==GEOMETRIE_LIGHT))
		{
			...
			{
        editace_textu=false;//v tomto případě musí být zde, což je prakticky konec
				smaz_usek_geometrie();
			}
			////načtení změny délky
			else
			{
				...
				else double_text=double_text*cos(asin(pom_element_temp->geo.HeightDepp/double_text))-pom_element_temp->geo.delkaPud;//zjištění posunu ve S/K
	...
	if(OBJEKT_akt!=NULL && OBJEKT_akt->teplomery!=NULL)
	{
		...
		if(T!=NULL)
		{
			...
			if(T->posledni->mGrid==NULL)
			{
				...
				T->posledni->mGrid->Tag=6;
				T->posledni->mGrid->ID=T->posledni->n;
			...
			if(T->prvni->sparovany->pohon!=NULL)
			{
				...
				if(T->prvni->sparovany->eID==0)
				{
					...
					{
						Cvektory::TCesta *c=d.v.vrat_segment_cesty(d.v.ZAKAZKA_akt,T->prvni->sparovany);
						if(c!=NULL)T->prvni->sparovany->data=c->data;//přepsání aktuálních dat ze zakázky do elementu
						c=NULL;delete c;
					}
			...
			while(CE!=NULL)
			{
				if(CE->Element->pohon!=NULL)
				{
		      	...
		      	if(d.v.ZAKAZKA_akt!=NULL && d.v.ZAKAZKA_akt->n!=0)//pokud pracuji v nějaké zakázce
						{
							Cvektory::TCesta *c=d.v.vrat_segment_cesty(d.v.ZAKAZKA_akt,CE->Element);
							if(c!=NULL)CE->Element->data=c->data;//přepsání aktuálních dat ze zakázky do elementu
              c=NULL;delete c;
						}
						//výpočet času
