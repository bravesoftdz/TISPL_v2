...
void __fastcall TForm1::FormKeyPress(TObject *Sender, System::WideChar &Key)
{
  log(__func__);//logování
	...
	if (editace_textu&&(index_kurzoru<=-11||index_kurzoru==-2||index_kurzoru==-8||index_kurzoru==-9))//editace kót elementů,kót haly nebo objektu, kót kabiny
	{
		if(Key==8)//pokud je stisknut backspace
		...
		if(!dblClick)//v případě, že se nejedná o double click
		{
			if(Button==mbLeft/* && MOD!=REZERVY*/)//je stisknuto levé tlačítko myši
			{
				...
				if(Akce==NIC && posun_objektu==false && funkcni_klavesa==0)//pokud není aktivovaná jiná akce
				{
					...
					{     if(JID==MaxInt){Akce=MOVE_USECKA;minule_souradnice_kurzoru=vychozi_souradnice_kurzoru;}//posun úsečky
...
//				}
		...
		case MOVE_KOMORA://posun komory vedoucí k jejímu odstranění
		{
			...
			if(m.P2Lx(X)<pom_temp->Xk||m.P2Lx(X)>pom_temp->Xk+pom_temp->rozmer_kabiny.x||m.P2Ly(Y)>pom_temp->Yk||m.P2Ly(Y)<pom_temp->Yk-pom_temp->rozmer_kabiny.y)Smazat1Click(this);
			nahled_ulozit(true);
			break;
		}
		...
		case MOVE_USECKA:
		{
			...
			if(pom_bod->n==1)//poslední úsečka
			{
				...
				if(pom!=NULL){A=pom->body->predchozi;B=pom->body->dalsi;}//posun úsečky objektu ve schématu
		...
		case ROZMER_KABINA: //připraveno pro implementaci kontroly zda jsou elementy v kabině
		{
			...
			if(JID==-4)
			{
				...
				if(pom_temp->id==3)pom_temp->komora->predchozi->velikost+=posunx;
			}
			if(JID==-5)
			{
				pom_temp->rozmer_kabiny.y-=posuny;
				if(pom_temp->elementy->X==0&&offset_spolus_rozmerem)pom_temp->koty_elementu_offset-=posuny;
				if(pom_temp->id==3)pom_temp->komora->predchozi->velikost+=posuny;
			}
			if(pom_temp->id==3)if(pom_temp->komora->predchozi->velikost<=0.5){pom_temp->komora->predchozi->velikost=0.5;pom_temp->rozmer_kabiny.x=d.v.vrat_velikosti_komor();Akce=NIC;}//nelze zmenšít kabinu tak, aby byla komora mimo ni!
      ...
      refresh_mGrid=true;
		...
		case OFFSET_KOTY:
		{
			...
			else//pro schéma
			{
				...
				Cvektory::TBod *Bod;
				if(pom_bod->n==1&&pom==NULL)Bod=d.v.HALA.body->predchozi;
				if(pom_bod->n==1&&pom!=NULL&&pom->body!=NULL)Bod=pom->body->predchozi;
...
//JID=-11 a dál;//hodnota kóty konkrétně a n elementu (10+pom_element->n)*(-1);hodnota kóty konkrétně a n komory (10+pom_komora->n)*(-1)
//JID=-1 žádná
  	...
  	if(JID==-1&&pom_temp->uzamknout_nahled==false&&pom_temp->id==3)
  	{
  		...
  		if(pom_komora!=NULL)//komora nalezena
  		{
  			...
  			if(pom_temp->rotace==0||pom_temp->rotace==180)
  				{if(X<=m.L2Px(pom_temp->Xk+d.v.vrat_velikosti_komor(pom_komora))+9&&X>=m.L2Px(pom_temp->Xk+d.v.vrat_velikosti_komor(pom_komora))-3)JID=(10+pom_komora->n)*(-1);}
  			else
  				{if(Y>=m.L2Py(pom_temp->Yk+d.v.vrat_velikosti_komor(pom_komora))-3&&Y<=m.L2Py(pom_temp->Yk+d.v.vrat_velikosti_komor(pom_komora))+3)JID=(10+pom_komora->n)*(-1);}
  		}
...
void TForm1::setJobIDOnMouseMove(int X, int Y)
{
	if(MOD==NAHLED)
	{
  	...
  	if(puvJID!=JID || pom_element_puv!=pom_element || pom_komora_puv!=pom_komora)//pokud došlo ke změně JID, nebo změně elementu bez změny JID (např. situace dva roboti vedle sebe nebo rychlý přesun), jinak nemá smysl řešit
  	{
  		...
  		if(JID==-5){kurzor(zmena_d_y);refresh_mGrid=false;}//kurzor pro zmenu velikosti kabiny
          ...
          C->Pen->Color=clWhite;//nastavení barev pro vykreslení obdelníku
			...
			case 5://objekt lakování
			{
	    		...
	    		if(n==1){ label1= "kontinuální"; label2="lakování"; }
					...
					if(pom_temp->pohon!=NULL&&EID==-1)
					{
						d.vykresli_element(C,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P+30-odsazeni,label1,label2,n);
					}
					else if(EID==-1) d.vykresli_element(C,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P+30-odsazeni,label1,label2,n,0,0,-1);
				}
				//změna vykreslení robotů v případě vložení elementu
		...
		default://SCHEMA
		{
			...
			else if(pom_bod_temp!=NULL)//mazání bodu haly nebo objektu
			{
				if(mrYes==MB(akt_souradnice_kurzoru_PX.x+10,akt_souradnice_kurzoru_PX.y+10,"Chcete opravdu smazat bod č. \""+AnsiString(pom_bod_temp->n),"",MB_YESNO))
				{
					...
					if(d.v.HALA.body!=NULL&&d.v.HALA.body->predchozi->n==2)d.v.smaz_bod(pom_bod_temp->predchozi,pom);
					if(pom!=NULL&&pom->body->predchozi->n==2)d.v.smaz_bod(pom_bod_temp->predchozi,pom);
					d.v.smaz_bod(pom_bod_temp,pom);
...
void TForm1::vykresli_kurzor(int index)
{
  log(__func__);//logování
	...
	switch ((index))//index=JID, kde a jaký kurzor vykreslit
	{
		...
		case -2://hodnota kót objektu a haly
		{
...
void TForm1::Smaz_kurzor()
{
  log(__func__);//logování
	...
	if(editace_textu&&pom_temp!=NULL)//ukončí editaci textu
	{
		...
		{
			editovany_text=inDK(ms.MyToDouble(editovany_text));
			pom_temp->rozmer_kabiny.x+=ms.MyToDouble(editovany_text)-pom_komora_temp->velikost;//zajištění, že kabina bude vždy přesně na komory
	...
	else if(editovany_text!="")//pokud neexistuje otevřený objekt + očetření proti zapsání prázdné hodnoty
	{
		//deklarace pomocných proměnných
		double puv,nov=inDK(ms.MyToDouble(editovany_text)),posun;
		Cvektory::TBod *A=pom_bod_temp->predchozi;
		if(pom_bod_temp->n==1&&pom==NULL)A=d.v.HALA.body->predchozi;
		if(pom_bod_temp->n==1&&pom!=NULL&&pom->body!=NULL)A=d.v.HALA.body->predchozi;
