...
{
	log(__func__);//logování
	if(MOD==NAHLED && Schema->ImageIndex!=-1)scButton_zamekClick(this);
	if(MOD==SCHEMA)scButton_zamek_layoutuClick(this);
	if(MOD!=SCHEMA && MOD!=NAHLED)
	{
...
void __fastcall TForm1::FormPaint(TObject *Sender)
{
  ...
  ///////zobrazení spojnice mezi tabulkou a elementem
	if(pom_temp!=NULL && pom_temp->zobrazit_mGrid && (JID==0 || JID==1 || JID==100 || 1000<=JID && JID<2000) && pom_element!=NULL && Akce==NIC)vykresli_spojinici_EmGrid(Canvas,pom_element);
}
//---------------------------------------------------------------------------
...
void TForm1::smaz_edit(bool refresh)
{
  log(__func__);//logování
	...
	{
		pom_element_temp=pom_temp->element;
			...
			if(Button==mbLeft/* && MOD!=REZERVY*/)//je stisknuto levé tlačítko myši
			{
				...
				if(Akce==NIC && posun_objektu==false && funkcni_klavesa==0)//pokud není aktivovaná jiná akce
				{
					if(MOD==NAHLED && pom_temp!=NULL)//TABULKA či ELEMENT
					{
						...
						//if(JID==-6) {if(scSplitView_LEFTTOOLBAR->Visible && scSplitView_LEFTTOOLBAR->Opened)DrawGrid_knihovna->SetFocus();stav_kurzoru=false;editace_textu=true;index_kurzoru=-6;nazev_puvodni=pom_temp->name;TimerKurzor->Enabled=true;}//editace názvu
						...
						if(JID<=-11&&JID>=-101&&pom_temp->id!=3){if(scSplitView_LEFTTOOLBAR->Visible && scSplitView_LEFTTOOLBAR->Opened)DrawGrid_knihovna->SetFocus();TimerKurzor->Enabled=true;editace_textu=true;stav_kurzoru=false;index_kurzoru=JID;pom_element_temp=pom_element;if(JID!=-101)editovany_text=m.round2double(d.v.vzdalenost_od_predchoziho_elementu(pom_element_temp),2);else editovany_text=m.round2double(vzdalenost_meziLO(pom_element,pom_temp->orientace),2);if(DKunit==2||DKunit==3)editovany_text=m.round2double(editovany_text/pom_temp->pohon->aRD,3);editovany_text=outDK(ms.MyToDouble(editovany_text));puv_souradnice.x=pom_element->X;puv_souradnice.y=pom_element->Y;}//editace kót elementu
						if(JID<=-11&&JID>=-101&&pom_temp->id==3){if(d.v.PtInKota_komory(pom_temp,X,Y)==-1){Akce=ROZMER_KOMORA;pom_komora_temp=pom_komora;minule_souradnice_kurzoru=vychozi_souradnice_kurzoru;}else {if(scSplitView_LEFTTOOLBAR->Visible && scSplitView_LEFTTOOLBAR->Opened)DrawGrid_knihovna->SetFocus();TimerKurzor->Enabled=true;editace_textu=true;stav_kurzoru=false;index_kurzoru=JID;pom_komora_temp=pom_komora;editovany_text=m.round2double(outDK(pom_komora->velikost),2);}}
	...
	switch(Akce)
	{
      ...
      //zapnutí kurzoru
		...
		case MOVE_USECKA:
		{
			...
			Cvektory::TElement *E_poslendi_pom_temp=d.v.vrat_posledni_element_objektu(pom_temp);
			if(pom!=NULL && pom_temp==NULL && pom->body->predchozi->n==4 && E_poslendi_pom->n==1 && E_poslendi_pom->geo.typ==0 && E_poslendi_pom->pohon==NULL || pom_temp!=NULL && pom_temp->body->predchozi->n==4 && E_poslendi_pom_temp->n==1 && E_poslendi_pom_temp->geo.typ==0 && E_poslendi_pom_temp->pohon==NULL)//speciální uprava objektu POW při posunu jeho přímek, z layoutu + z editace
			{
...
//          }
			...
			minule_souradnice_kurzoru=TPoint(X,Y);
			REFRESH();
			E_poslendi_pom_temp=NULL;delete E_poslendi_pom_temp;
...
//		}
		...
		case GEOMETRIE:
		{
			if(Akce_temp==NIC)
			{
				Cvektory::TElement *e_poslendi=d.v.vrat_posledni_element_objektu(pom_temp);
				...
				if(pom->dalsi!=NULL && e_poslendi->geo.X4!=pom->dalsi->element->geo.X1 && e_poslendi->geo.X4!=pom->dalsi->element->geo.X1)
		  	{
				...
				else if(m.PtInCircle(akt_souradnice_kurzoru.x,akt_souradnice_kurzoru.y,pom_temp->element->geo.X1,pom_temp->element->geo.Y1,0.3) && posledni_editovany_element!=NULL){posledni_editovany_element=NULL;editace_geometrie_spustena=true;}
...
void TForm1::vykresli_spojinici_EmGrid(TCanvas *Canv,Cvektory::TElement *E)
{
	...
	if(pom_temp!=NULL && pom_temp->zobrazit_mGrid && E!=NULL && (d.v.vrat_druh_elementu(E)!=-1 || E->eID==200) && E->mGrid!=NULL)
	{
	...
	if(Button==mbLeft)//zohlední jenom stisk levého tlačítka myši
	{
		...
		switch(Akce)
		{
			...
			case MOVE:
			{
				...
				if(pom_temp==NULL && pom->element->pohon!=NULL && predchozi_souradnice_kurzoru.x!=m.L2Px(pom->element->geo.X1) && predchozi_souradnice_kurzoru.y!=m.L2Px(pom->element->geo.Y1) && mrNo==MB(akt_souradnice_kurzoru_PX.x+10,akt_souradnice_kurzoru_PX.y+10,ls->Strings[416],"",MB_YESNO,true,false))//"Objekt byl přesunut, souhlasíte s aktuálním umístěním?"
			...
			case MOVE_TEXT:if(pom_temp!=NULL && vychozi_souradnice_kurzoru.x==minule_souradnice_kurzoru.x && vychozi_souradnice_kurzoru.y==minule_souradnice_kurzoru.y){if(scSplitView_LEFTTOOLBAR->Visible && scSplitView_LEFTTOOLBAR->Opened)DrawGrid_knihovna->SetFocus();stav_kurzoru=false;editace_textu=true;index_kurzoru=-6;nazev_puvodni=pom_temp->name;TimerKurzor->Enabled=true;}Akce=NIC;kurzor(standard);pom_vyhybka=NULL;break;
			case MOVE_BOD:
...
void TForm1::getJobID(int X, int Y)
{
	...
	if(MOD==NAHLED && !pom_temp->uzamknout_nahled)
	{
		...
		if(JID==-1)//jiný objekt než objekt editovaný
		{
			...
			if(O!=NULL && O->n!=pom_temp->n)JID=-203;
			O=NULL;delete O;
    }
	}
...
//		}
	...
	if(JID==-1 && (d.v.PP.zamek_layoutu || pom_temp!=NULL && pom_temp->uzamknout_nahled))//hledání citelné oblasti zprávy, vždy vyhledávat(i při zamčeném layoutu či editaci)!!
	{
		d.zprava_highlight=d.v.PtInZpravy();
...
void TForm1::onPopUP(int X, int Y)
{
	...
	//rotace názvu
	if(pom_temp!=NULL || JID==-6){otocit_doleva=ls->Strings[404];otocit_doprava=ls->Strings[405];}
...
{																																																																								 																									//"Objekty nenavazují, automaticky posunout následující objekt pro zachování návaznosti?"
	log(__func__);//logování
	Cvektory::TElement *e_posledni=d.v.vrat_posledni_element_objektu(pom_temp);
...
void TForm1::add_element (int X, int Y)
{
    ...
    if(E!=NULL && (E->eID==5 || E->eID==6))FormX->validace_max_voziku();//přidána otoč do kabiny, aktualizace validace
		...
		if(E!=NULL && E->eID==200)
		{
			...
			while(E_temp!=NULL)//odmazání pohonu za zarážkou
			{
				E_temp->pohon=NULL;
				set_enabled_mGrid(E_temp);
				E_temp=E_temp->dalsi;
			}
			delete E_temp;E_temp=NULL;
...
{                 //stejné komory přidat jejich šířku jinak 2.5
	...
	else//přidání nové komory
	{
  	...
  	double rozmer_kabiny=pom_temp->element->geo.delka,posun_x=0,posun_y=0;
  	switch((int)pom_temp->orientace)
  	{
  	...
  	//rozšíření pohonu
  	pom_temp->element->X+=posun_x;
  	pom_temp->element->Y+=posun_y;
  	d.v.vloz_G_element(pom_temp->element,0,pom_temp->element->geo.X1,pom_temp->element->geo.Y1,0,0,0,0,pom_temp->element->X,pom_temp->element->Y,pom_temp->element->geo.orientace);
	}
  //ostatní
	Akce=NIC;
	refresh_mGrid=false;
...
void TForm1::vlozit_predavaci_misto()
{
	...
	while(O!=NULL)
	{
		...
		if(pom_temp!=NULL && O->n==pom_temp->n)E=pom_temp->element;
		while(E!=NULL)
		{
			//deklarace atributů
			long ID=0;
			...
			if(E->eID!=200 && ((E->dalsi!=NULL && E->dalsi->pohon!=NULL && E->pohon!=NULL && E->dalsi->pohon->n!=E->pohon->n) || (E->dalsi==NULL && O->dalsi!=NULL && O_pom->element->pohon!=NULL && E->pohon!=NULL && O_pom->element->pohon->n!=E->pohon->n) || (E->dalsi!=NULL && (E->dalsi->pohon==NULL && E->pohon!=NULL || E->dalsi->pohon!=NULL && E->pohon==NULL)) || (E->dalsi==NULL && O->dalsi!=NULL && (O_pom->element->pohon==NULL && E->pohon!=NULL || O_pom->element->pohon!=NULL && E->pohon==NULL))))
			{
				//WT přiřazení
				...
				if(E->dalsi==NULL && O->dalsi!=NULL && O_pom->element->pohon!=NULL)E->WT=m.cekani_na_palec(0,O_pom->element->pohon->roztec,O_pom->element->pohon->aRD,3);
				//souřadnice tabulky
...
//				E->short_name=E->name.SubString(1,3)+AnsiString(nTyp);
			...
			if(E->eID==200 && (E->dalsi!=NULL && E->dalsi->pohon==NULL && E->pohon==NULL || E->dalsi!=NULL && E->pohon!=NULL && E->dalsi->pohon!=NULL && E->dalsi->pohon->n==E->pohon->n || E->dalsi==NULL && O->dalsi!=NULL && E->pohon!=NULL && O_pom->element->pohon!=NULL && O_pom->element->pohon->n==E->pohon->n || E->dalsi==NULL && O->dalsi!=NULL && O_pom->element->pohon==NULL && E->pohon==NULL))
			{
			...
			if(E->eID==200 && (E->dalsi!=NULL && E->dalsi->eID==200 && E->dalsi->geo.delka<0.01 || E->dalsi==NULL && O->dalsi!=NULL && O_pom->element->eID==200 && O_pom->element->geo.delka<0.01))
			{
        //posun na druhé předávací místo
				if(E->dalsi!=NULL)E=E->dalsi;
				else E=O_pom->element;
      	//pokud je možné dojde k odstranění elementu
				if(E->dalsi!=NULL && E->dalsi->geo.typ==0 && E->geo.typ==0){d.v.smaz_element(E);break;}
				//pokud ne je přepnut na zarážku
				else
	...
	if(d.v.OBJEKTY->dalsi!=NULL && d.v.OBJEKTY->predchozi->n>=3)
	{
		...
		Cvektory::TElement *e_posledni=d.v.vrat_posledni_element_objektu(O_posledni);
		//////////Vkládání předávacího místa
		if(O->element->pohon!=NULL && e_posledni->pohon==NULL || O->element->pohon==NULL && e_posledni->pohon!=NULL || O->element->pohon!=NULL && e_posledni->pohon!=NULL && O->element->pohon->n!=e_posledni->pohon->n)
		{
			Cvektory::TElement *E=e_posledni;
			//WT přiřazení
			E->WT=0;//čekání na palec
			if(O->element->pohon!=NULL)E->WT=m.cekani_na_palec(0,O->element->pohon->roztec,O->element->pohon->aRD,3);
			//souřadnice tabulky
		...
		//////////Aktualizace WT
		e_posledni=d.v.vrat_posledni_element_objektu(O_posledni);
		if(e_posledni->eID==200)
		{
			if(O->element->pohon!=NULL)e_posledni->WT=m.cekani_na_palec(0,O->element->pohon->roztec,O->element->pohon->aRD,3);
			if(pom_temp!=NULL && pom_temp->n==O_posledni->n)e_posledni->mGrid->Refresh();
		}
		//////////Mazání nepotřebného předávacího místa (změna na zarážku)
		if(e_posledni->eID==200 && (O->element->pohon==NULL && e_posledni->pohon==NULL || O->element->pohon!=NULL && e_posledni->pohon!=NULL && O->element->pohon->n==e_posledni->pohon->n))
		{
			//vynulování WT
			e_posledni->WT=0;
			//změna na zarážku
...
void TForm1::vlozeni_editace_geometrie()
{
  ...
  //////deklarace atributů
	...
	//////nastavení orientace
	short orientace=0;
	if(posledni_editovany_element!=NULL){orientace=posledni_editovany_element->orientace;}else {orientace=d.v.vrat_posledni_element_objektu(pom_temp)->orientace;}
	...
	if(posledni_editovany_element==NULL)
	{
    	...
    	posunx=d.geoTemp.X4-d.geoTemp.X1;posuny=d.geoTemp.Y4-d.geoTemp.Y1;
			posledni_editovany_element=d.v.vloz_element(pom_temp,MaxInt,d.geoTemp.X4,d.geoTemp.Y4,orientace,pom_temp->element);
		...
		{
//			short rot=m.Rt90(pom_temp->element->orientace-pom_temp->element->geo.orientace-pom_temp->element->geo.rotacni_uhel);
//			rot=m.Rt90(d.geoTemp.orientace+d.geoTemp.rotacni_uhel+rot);  //element se nerotuje okolo konečného bodu ramena
			posunx=d.geoTemp.X4-pom_temp->element->geo.X4;posuny=d.geoTemp.Y4-pom_temp->element->geo.Y4;
			pom_temp->element->geo=d.geoTemp;
			pom_temp->element->X+=posunx;pom_temp->element->Y+=posuny;
			posledni_editovany_element=pom_temp->element;
...
void TForm1::ukonceni_geometrie()
{
	...
	{
		Cvektory::TElement *E1=d.v.vrat_posledni_element_objektu(pom_temp),*E2=pom->dalsi->element;
...
void TForm1::mGrid_on_mGrid()
{
	...
	if(pom_temp!=NULL && pom_temp->zobrazit_mGrid)//pokud existuje editovaný objekt a jsou zobrazeny mGridy
	{
		...
		Cvektory::TElement *E=pom_temp->element,*prekryty=NULL;
...
{        
	...
	if(pom_temp!=NULL && E!=NULL && d.v.vrat_druh_elementu(E)!=-1)
	{
		...
		if(!prekryti)
		{
			...
			Cvektory::TElement *e_posledni=d.v.vrat_posledni_element_objektu(pom_temp);
			if(pom_temp->element->geo.orientace==E->geo.orientace)dalsi=true;
			if(e_posledni->geo.orientace==E->geo.orientace)predchozi=true;
			switch((int)E->geo.orientace)
			{
				case 0:if(dalsi && el1.bottom>m.L2Py(pom_temp->element->geo.Y1) || predchozi && m.L2Py(e_posledni->geo.Y4)>el1.top)prekryti=true;break;
				case 90:if(dalsi && el1.left<m.L2Px(pom_temp->element->geo.X1) || predchozi && m.L2Px(e_posledni->geo.X4)<el1.right)prekryti=true;break;
				case 180:if(dalsi && el1.top<m.L2Py(pom_temp->element->geo.Y1) || predchozi && m.L2Py(e_posledni->geo.Y4)<el1.bottom)prekryti=true;break;
				case 270:if(dalsi && el1.right>m.L2Px(pom_temp->element->geo.X1) || predchozi && m.L2Px(e_posledni->geo.X4)>el1.left)prekryti=true;break;
			}
			e_posledni=NULL;delete e_posledni;
...
void TForm1::vytvoreni_tab_pohon()
{
  log(__func__);//logování
	...
	else
	{
		switch(pom_temp->rezim)//tabulka má 7 řádků
   	{
   		...
   		case 1://kontinuální režim, zde jsou možné další 2 možnosti, 6 nebo 7 řádků
			{
				if(pocet_vyskytu_elementu_s_otoci(pom_temp)>0);
	...
	{
		double uhel=m.Rt90(d.v.vrat_rotaci_jigu_po_predchazejicim_elementu(pom_temp->element)+pom_temp->element->rotace_jig);
		...
		case 1://smazání elementu
		{
			...
			else
			{
		  	//odebrán poslední element s otočí, ale zobrazeny obě mezery mezi jig
				if(pocet_vyskytu_elementu_s_otoci(pom_temp)==0&&PmG->Rows[7].Visible)
					PmG->VisibleRow(7,false,false);
  		...
  		while (P!=NULL)
			{
				AnsiString dopRD="";
				t=Combo->Items->Add(/*tady nelze parametr*/);
				if(Combo->ItemIndex==(long)P->n && pom_temp->pohon!=NULL && pom_temp->pohon->n==P->n)//pokud se jedná o aktuální pohon a pohon lze editovat a neprobíhá změna comboboxu a již existuje nějaký pohon
  			{
...
void TForm1::prvni_vytvoreni_tab_elementu (Cvektory::TElement *E,short sirka_0,short sirka_1,short sirka_2,short sirka_3,short sirka_4,short sirka_56,short sirka_cisla,AnsiString LO,AnsiString cas,AnsiString delka_otoce)
{
	...
	switch(E->eID)
	{
		case 0://stop stanice
		{
			...
			if(id>=6 && id<=10){popisek=ls->Strings[327]+" "+pom_temp->name.LowerCase();hint=ls->Strings[417]+" "+pom_temp->name.LowerCase()+" "+ls->Strings[418];}//čas "název objektu",maximální čas "název objektu" včetně max. WT palec
			//definice buněk
...
void TForm1::redesign_element()
{
  log(__func__);//logování
	...
	Cvektory::TElement *E=pom_temp->element;//zde lze přeskočit hlavičku
	while (E!=NULL)
	{
...
void TForm1::akt_tabulek (Cvektory::TElement *E,AnsiString LO,AnsiString delka_otoce,AnsiString cas,short sirka_0,short sirka_1,short sirka_2,short sirka_3,short sirka_4,short sirka_56,short sirka_cisla)
{
	...
	switch(E->eID)
	{
		case 0://stop stanice
		{
			...
			if(pom_temp->id>=6 && pom_temp->id<=10)popisek=ls->Strings[327]+" "+pom_temp->name.LowerCase();//čas "název objektu"
			E->mGrid->Cells[0][2].Text="RT "+cas;
...
{
	log(__func__);//logování
	if(MOD==NAHLED && pom!=NULL && pom_temp!=NULL)//ošetření navíc
	{
		...
		if(pom_temp->id!=3 && (DrawGrid_knihovna->ColCount!=2 || DrawGrid_knihovna->DefaultColWidth!=80)){DrawGrid_knihovna->ColCount=2;DrawGrid_knihovna->DefaultColWidth=80;}//ošetření probliku
		if(pom_temp->id==3 && (DrawGrid_knihovna->ColCount!=1 || DrawGrid_knihovna->DefaultColWidth!=160)) {DrawGrid_knihovna->ColCount=1;DrawGrid_knihovna->DefaultColWidth=160;}
    ...
    //překreslení původních
		...
		switch(pom_temp->id)
		{
			...
			case 0:case 9://objekt navěšování + svěšování
			{
				...
				if(pom_temp->id==9)label_pom=ls->Strings[255];//"svěšování"
				...
				if(pom_temp->rezim==1 && pom_temp->pohon!=NULL)
				{
				...
				else if (pom_temp->rezim==0 && pom_temp->pohon!=NULL)
				{
          ...
          int X, int Y)
{
	...
	if(MOD==NAHLED)
  {
    ...
    //pokud byly splněné podmínky pro přidání, zapne událost ADD (vystrčeno mimo z důvodu množení kódu)
		...
		{
			Akce=NIC;kurzor(standard);
			if(pom_temp->pohon!=NULL)TIP=ls->Strings[310];//"Přidávání prvků je možné až po výběru pohonu"
			...
			if(Akce!=GEOMETRIE && pom_element!=NULL && pom_element->eID!=MaxInt)//případ mazání běžného elementu
			{
				if(mrYes==MB(akt_souradnice_kurzoru_PX.x+10,akt_souradnice_kurzoru_PX.y+10,text+pom_element_temp->name.UpperCase()+"?","",MB_YESNO))
				{
		  		...
		  		DrawGrid_knihovna->Refresh();
					...
					if(pom_temp->element!=NULL)d.v.uprav_popisky_elementu(pom_temp->element);//pokud jsou v kabině jěště nějaké elementy dojde k přejmenování
...
void __fastcall TForm1::NastavitparametryClick1Click(TObject *Sender)
{
	...
	if(pom_temp!=NULL && pom_vyhybka!=NULL && pom_temp->n!=pom_vyhybka->n && pom_bod_temp==NULL)//otevírání náhledu z náhledu, přechot na editaci jiného objektu
	{
...
void TForm1::NP_input()
{
	 ...
	 pom_temp=new Cvektory::TObjekt; pom_temp->pohon=NULL; pom_temp->pohon=new Cvektory::TPohon; pom_temp->element=NULL;
   ...
   if(zmena_zoom)
   {
	 ...
	 if(pom_temp->id==4 || pom_temp->id==2 || pom_temp->id==5)scGPLabel_roboti->Caption=ls->Strings[55];//"Robot           Operátor";//mezery tvoří místo, kde je zobrazen switch
	 else if(pom_temp->id==3)scGPLabel_roboti->Caption=ls->Strings[56];//"Sekce";
	 else if(pom_temp->id==0 || pom_temp->id==9)scGPLabel_roboti->Caption=ls->Strings[57];//"Operátoři";
	...
	mGrid_on_mGrid();
	if(pom_temp->pohon!=NULL)aktualizace_ComboPohon();//aktualizace comba pokud je pohon přiřazen
	pom_element_temp=pom_temp->element;//pro pořeby editace geometrie
...
void TForm1::zmena_editovaneho_objektu()
{
    ...
    /////////Vrácení knihoven a jejich popisků na default pozice
  	...
  	if(pom_temp->id==4 || pom_temp->id==2 || pom_temp->id==5){scGPPanel_pomocn_proSwitch->Visible=true;scGPSwitch_robot_clovek->Visible=true;}
		...
		//nastavení záložky
		Schema->Caption=ls->Strings[36]+" "+pom->name.UpperCase();
		if(pom_temp->uzamknout_nahled)Schema->ImageIndex=79;
	  else Schema->ImageIndex=-1;
...
//		}
	if(pom_temp!=NULL)//pro případ, že uživatel změní hodnotu v tabulce a než se stihne provést validace ukončí editaci
	{
...
void TForm1::vykresli_kurzor(int index)
{
  log(__func__);//logování
	...
	if(pom_temp!=NULL)
	{
		...
		d.nastavit_text_popisu_objektu_v_nahledu(Canvas);Tz=F->pom_temp->short_name.UpperCase();
	}
  ...
  log(__func__);//logování
    ...
    //pokud je kurzor vykreslen překresli ho
		...
		if(index_kurzoru<=-11&&pom_temp->id==3&&Akce!=GEOMETRIE)//editace rozmeru komor v POW
		{
			...
			if(pom_temp->orientace==90 || pom_temp->orientace==270)pom_temp->element->X+=rozdil;
			else pom_temp->element->Y+=rozdil;
			d.v.vloz_G_element(pom_temp->element,0,pom_temp->element->geo.X1,pom_temp->element->geo.Y1,0,0,0,0,pom_temp->element->X,pom_temp->element->Y,pom_temp->element->geo.orientace);
		}
...
unsigned short TForm1::load_language(Tlanguage language,bool akt_mGrid)
{
  ...
  ChDir(ExtractFilePath(Application->ExeName));    //přesune k EXE
    ...
    Konec->Caption=ls->Strings[19];
		if(pom_temp==NULL)Schema->Caption=ls->Strings[20];
