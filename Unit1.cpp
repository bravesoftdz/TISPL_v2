...
//	scListGroupKnihovObjektu->Left=0;
 /*	scListGroupKnihovObjektu->Top=2+scListGroupNastavProjektu->Height+0;
  ...
  //dočasné vypnutí záložky editace
	...
	ls=new TStringList;
	UnicodeString text="";
	for(unsigned short i=0;i<=422;i++)
	{
		switch(i)
		{
			...
			if(Button==mbLeft/* && MOD!=REZERVY*/)//je stisknuto levé tlačítko myši
			{
				...
				if(Akce==NIC && posun_objektu==false && funkcni_klavesa==0)//pokud není aktivovaná jiná akce
				{
					if(MOD==NAHLED && pom_temp!=NULL)//TABULKA či ELEMENT
					{
						...
						if(1000<=JID && JID<2000 || JID>2000 && JID<3000){Akce=MOVE_TABLE;kurzor(posun_l);minule_souradnice_kurzoru=vychozi_souradnice_kurzoru;pom_element->mGrid->Highlight;refresh_mGrid=false;d.nabuffrovat_mGridy();}//TABULKA posun
...
void __fastcall TForm1::FormDblClick(TObject *Sender)
{
	...
	else//jsem v náhledu
	{
		...
		if(Akce==NIC)
		{
			...
			if(pom_vyhybka==NULL && pom_temp!=NULL)KonecClick(this);
		}
	}
}
//---------------------------------------------------------------------------
...
void TForm1::vykresli_spojinici_EmGrid(TCanvas *Canv,Cvektory::TElement *E)
{
	...
	if(pom_temp->zobrazit_mGrid && E->mGrid->Rows[0].Visible)
	{
	...
	if(MOD==NAHLED)
	{
      ...
      if(JID==-102)kurzor(info);//zprávy
  		...
  		if(JID==1000){pom_element->mGrid->CheckLink(X,Y);refresh_mGrid=true;}//pouze pro přechod název hlavička, aby název nezůstal tučně - aktivace dodáním pouze aktuálních souřadnic
			if(1000<=JID && JID<2000 || JID>2000 && JID<3000){kurzor(posun_ind);pom_element->mGrid->Highlight=true;pom_element->mGrid->MouseMove(X,Y);refresh_mGrid=true;}//indikace posunutí TABULKY, jeji highlignutí probíhá výše a případné volání HINTu
...
//	delete E;E=NULL;
	d.line(Canvas,E->mGrid->LEFT,E->mGrid->Top,akt_souradnice_kurzoru_PX.x,akt_souradnice_kurzoru_PX.y);
}
//---------------------------------------------------------------------------
