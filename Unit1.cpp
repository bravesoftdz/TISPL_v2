	...
	else
	{
		if(!dblClick)//v případě, že se nejedná o double click
		{
            ...
            //nové JID pro objekt
						...
						if(JID==-201){OBJEKT_akt->pohon=element_temp->pohon;if(OBJEKT_akt->pohon!=NULL){prirazeni_pohonu_tab_pohon(OBJEKT_akt->pohon->n);FormX->aktualizace_PmG(true);}else {prirazeni_pohonu_tab_pohon(0);PmG->getCombo(0,0)->DropDown();}if(!OBJEKT_akt->zobrazit_mGrid)scGPButton_viditelnostmGridClick(Sender);} //kliknutí na jeden z pohonů na předávacím místě
						if(JID==-202){if(element_temp->dalsi!=NULL && element_temp->dalsi->objekt_n==element_temp->objekt_n){OBJEKT_akt->pohon=element_temp->dalsi->pohon;if(OBJEKT_akt->pohon!=NULL){prirazeni_pohonu_tab_pohon(OBJEKT_akt->pohon->n);FormX->aktualizace_PmG(true);}else {prirazeni_pohonu_tab_pohon(0);PmG->getCombo(0,0)->DropDown();}if(!OBJEKT_akt->zobrazit_mGrid)scGPButton_viditelnostmGridClick(Sender);}else {if(element_temp->dalsi!=NULL)pom_vyhybka=d.v.vrat_objekt(element_temp->dalsi->objekt_n);else pom_vyhybka=d.v.vrat_objekt(d.v.ELEMENTY->dalsi->objekt_n);zmena_editovaneho_objektu();}}
					}
					else
					{
...
//						}
      ...
      //zapnutí kurzoru
			...
			if(OBJEKT_akt==NULL)
			{
				/*if(OBJEKT_akt==NULL)*/d.v.posun_objekt(akt_souradnice_kurzoru.x-m.P2Lx(minule_souradnice_kurzoru.x),akt_souradnice_kurzoru.y-m.P2Ly(minule_souradnice_kurzoru.y),pom);
				minule_souradnice_kurzoru=TPoint(X,Y);
				REFRESH(false);
			}else Akce=NIC;
			break;
...
{              
	...
	if(Button==mbLeft)//zohlední jenom stisk levého tlačítka myši
	{
		...
		if(pan_non_locked){pan_non_locked=false;Akce=NIC; kurzor(standard);pan_move_map();if(OBJEKT_akt!=NULL)mGrid_on_mGrid();}//kontrola, zda nejsou překryty mGridy elementů a PmG
		switch(Akce)
		{
	...
	if(Button==mbMiddle && Akce==PAN_MOVE)//ukončení posunu pomocí stisknutého kolečka
	{
		...
		kurzor(standard);
		pan_move_map();
		if(OBJEKT_akt!=NULL)mGrid_on_mGrid();//kontrola, zda nejsou překryty mGridy elementů a PmG
	}
	stisknute_leve_tlacitko_mysi=false;
...
void TForm1::setJobIDOnMouseMove(int X, int Y)
{
	...
	if(MOD==EDITACE)
	{
  		...
  		if(JID==-1 || pom_element==NULL || pom_element_puv!=pom_element)//není již job ID nebo ukazatel na pohon (může nastat situace přechod tabulka citelná oblast kót či nadpisu kabiny a nic by se bez tohoto nestalo
			{
				try//může se stát, že k tomuto dojde v době načítání z datového obrazu ... proto ošetření
				{
					...
					if(puvJID>=4 && puvJID<=10 && OBJEKT_akt->zobrazit_mGrid)PmG->MouseMove(X,Y);//najistotu hintů a zrušení highlignutí tabulky pohonu dodáním pouze aktuálních souřadnic
      	}catch(...){;}
			}
      ...
      if(JID==-6)kurzor(editace_posun);
			...
			if(JID>=11 && JID<=99){if(OBJEKT_akt->orientace==90||OBJEKT_akt->orientace==270)kurzor(zmena_d_y);else kurzor(zmena_d_x);refresh_mGrid=false;}//interaktivní kóty elementů
...
//  			E->short_name=E->name.SubString(1,3)+AnsiString(nTyp);
		...
		if(E->eID==300)
		{
    		...
    		if(p1!=NULL && p2!=NULL && p1!=p2)
    		{
				...
				//zapsání do mgridu
				if(OBJEKT_akt!=NULL && E->objekt_n==OBJEKT_akt->n)E->mGrid->Cells[2][11].Text = F->m.round2double(F->outPT(E->WT),3);
			}
			//ukazatelové záležitosti
		...
		if(E->eID==301)
		{
			...
			if(C!=NULL)
			{
				...
				//zapsání do mgridu
				if(OBJEKT_akt!=NULL && E->objekt_n==OBJEKT_akt->n)E->mGrid->Cells[1][2].Text = F->m.round2double(F->outPT(E->WT),3);
			}
			//ukazatelové záležitosti
			p1=NULL;p2=NULL;C=NULL;
			delete p1;delete p2;delete C;
	    	...
	    	if(E->pohon!=NULL)T1=E->pohon->name;
				...
				switch((int)E->orientace)
				{
					case 0:case 180:
					{
						...
						if(m.PtInRectangle(x1,y1,x1+w1,y1+h,X,Y)){ret=1;if(OBJEKT_akt->orientace==270)ret=2;element_temp=E;break;}
						if(m.PtInRectangle(x2,y2,x2+w2,y2+h,X,Y)){ret=2;if(OBJEKT_akt->orientace==270)ret=1;element_temp=E;break;}
					}break;
					case 90:
					{
...
void TForm1::vytvoreni_tab_pohon()
{
	...
	if(OBJEKT_akt->pohon!=NULL)
	{
		...
		PmG->Cells[1][2].Type=PmG->EDIT;
		PmG->Cells[1][2].Text=m.round2double(outR(OBJEKT_akt->pohon->roztec),3);
		...
		PmG->Cells[1][3].Text=m.round2double(OBJEKT_akt->pohon->Rx,3);
		PmG->Cells[0][4].Text=ls->Strings[211]+Rz;//"Rozteč jigů"
		OBJEKT_akt->pohon->Rz=m.Rz(F->OBJEKT_akt->pohon->aRD);
		PmG->Cells[1][4].Text=m.round2double(outRz(OBJEKT_akt->pohon->Rz),3);
		PmG->Cells[0][5].Text=ls->Strings[212]+Rz;//"Mezera mezi podvozky"
		PmG->Cells[1][5].Text=m.round2double(outRz(m.mezera(0,OBJEKT_akt->pohon->Rz,0)),3);
		PmG->Cells[0][6].Text=ls->Strings[213]+Rz;//"Mezera mezi jigy 0°"
		PmG->Cells[1][6].Text=m.round2double(outRz(m.mezera(0,OBJEKT_akt->pohon->Rz,1)),3);
		PmG->Cells[0][7].Text=ls->Strings[214]+Rz;//"Mezera mezi jigy 90°
		PmG->Cells[1][7].Text=m.round2double(outRz(m.mezera(90,OBJEKT_akt->pohon->Rz,1)),3);
	}
	...
	if(OBJEKT_akt->pohon==NULL)//pokud není přiřazen pohon zobrazí se pouze COMBO
	{
	...
	{
		switch(OBJEKT_akt->rezim)//tabulka má 7 řádků
   	{
   		...
   		case 1://kontinuální režim, zde jsou možné další 2 možnosti, 6 nebo 7 řádků
			{
				if(pocet_vyskytu_elementu_s_otoci(OBJEKT_akt)>0);
	...
	{
		double uhel=m.Rt90(d.v.vrat_rotaci_jigu_po_predchazejicim_elementu(OBJEKT_akt->element)+OBJEKT_akt->element->rotace_jig);
		PmG->Cells[0][6].Text=ls->Strings[215]+" "+AnsiString(uhel)+"°"+Rz;//"Mezera mezi jigy "
		PmG->Cells[1][6].Text=m.round2double(outRz(m.mezera(uhel,OBJEKT_akt->pohon->Rz,1)),3);
	}
	//finální desing
	...
	tab_pohon_COMBO(0);
	//PmG->Note.Text="";
	if(OBJEKT_akt->pohon!=NULL && d.v.pohon_je_pouzivan(OBJEKT_akt->pohon->n,false))//kontrola zda je pohon používán v jiném objektu, nutné posílat pom místo OBJEKT_akt do parametru mimo_objetk!!!!!
	{
...
void TForm1::pridani_elementu_tab_pohon(Cvektory::TElement *E)
{
  log(__func__);//logování
	...
	if(OBJEKT_akt->rezim==1&&!PmG->Rows[3].Visible)//změny pouze při přidání kontinuálního robota, vytvoření KK tabulky
	{
	...
	if(d.v.pohon_je_pouzivan(OBJEKT_akt->pohon->n,false))//kontrola zda je pohon používán v jiném objektu, nutné posílat pom místo OBJEKT_akt do parametru mimo_objetk!!!!!
	{
...
void TForm1::prirazeni_pohonu_tab_pohon(unsigned int index_pohonu)
{
  log(__func__);//logování
	...
	if(index_pohonu!=0)//pokud je přiřazen nějaký pohon
	{
		...
		if(OBJEKT_akt->id==3)OBJEKT_akt->rezim=1;//pro objekt POW bude vždy zobrazena tabulka v KK režimu
		switch(OBJEKT_akt->rezim)//tabulka má 7 řádků
   	{
   		case -1://nic, stejné jako S&G
			case 0://S&G, tři řádky
	...
	PmG->Update();
	if(JID==-201 || JID==-202)//při kliku na předávací místo (tj. změna editovaného pohonu) kontrola zda je pohon v tabulce uložen i v OBJEKT_akt
	{
		if(index_pohonu==0 && OBJEKT_akt->pohon!=NULL)OBJEKT_akt->pohon=NULL;
		if(index_pohonu>0 && (OBJEKT_akt->pohon!=NULL && index_pohonu!=OBJEKT_akt->pohon->n || OBJEKT_akt->pohon==NULL))d.v.kopiruj_pohon(d.v.vrat_pohon(index_pohonu),OBJEKT_akt);
	}
	if(OBJEKT_akt->pohon!=NULL)
	{
		if(d.v.pohon_je_pouzivan(OBJEKT_akt->pohon->n,false))//kontrola zda je pohon používán v jiném objektu, nutné posílat pom místo OBJEKT_akt do parametru mimo_objetk!!!!!
		...
		case 1://smazání elementu
		{
			...
			else
			{
		  	//odebrán poslední element s otočí, ale zobrazeny obě mezery mezi jig
				if(pocet_vyskytu_elementu_s_otoci(OBJEKT_akt)==0&&PmG->Rows[7].Visible)
					PmG->VisibleRow(7,false,false);
...
//		}break;
	...
	//změna v tabulce
	PmG->Cells[0][1].Text=ls->Strings[208]+aRD;//"Rychlost"
	PmG->Cells[1][1].Text=m.round2double(outaRD(OBJEKT_akt->pohon->aRD),3);//rychlost raději zaokrouhlovat
	PmG->Cells[0][2].Text=ls->Strings[209]+R;//"Rozteč palce"
	PmG->Cells[1][2].Text=m.round2double(outR(OBJEKT_akt->pohon->roztec),3);
	PmG->Cells[0][4].Text=ls->Strings[211]+Rz;//"Rozteč jigů"
	PmG->Cells[1][4].Text=m.round2double(outRz(OBJEKT_akt->pohon->Rz),3);
...
void TForm1::tab_pohon_COMBO (int index)
{
	...
	if(index==1 && JID!=-201 && JID!=-202)//přiřazení pohonu
	{
		...
		if(OBJEKT_akt->pohon!=NULL)p_puvodni=OBJEKT_akt->pohon->n;
		int pohon=PCombo->ItemIndex;
		//zapsání pohonu do aktuálně editovaného
		if(pohon!=0)OBJEKT_akt->pohon=d.v.vrat_pohon(pohon);//d.v.kopiruj_pohon(d.v.vrat_pohon(pohon),OBJEKT_akt);
		else OBJEKT_akt->pohon=NULL;
		Cvektory::TElement *E=OBJEKT_akt->element;
		//přiřazení pohonu elementům
		while(E!=NULL && E->objekt_n==OBJEKT_akt->n)
		{
			if(E->pohon==NULL && p_puvodni==0 || E->pohon!=NULL && p_puvodni!=0 && E->pohon->n==p_puvodni){E->pohon=OBJEKT_akt->pohon;}
			set_enabled_mGrid(E);
		...
		vlozit_predavaci_misto_aktualizuj_WT();//přidání či odebrání předávacího místa
  		...
  		while (P!=NULL)
			{
				AnsiString dopRD="";
				t=Combo->Items->Add(/*tady nelze parametr*/);
				if(Combo->ItemIndex==(long)P->n && OBJEKT_akt->pohon!=NULL && OBJEKT_akt->pohon->n==P->n)//pokud se jedná o aktuální pohon a pohon lze editovat a neprobíhá změna comboboxu a již existuje nějaký pohon
  			{
  				t->Caption=F->OBJEKT_akt->pohon->name+" - "+F->m.round2double(F->outaRD(F->OBJEKT_akt->pohon->aRD),3,"..")+" ("+AnsiString(F->m.round2double(F->outaRD(F->OBJEKT_akt->pohon->rychlost_od),2))+"-"+AnsiString(F->m.round2double(F->outaRD(F->OBJEKT_akt->pohon->rychlost_do),2))+") "+caption_jednotky;//vypíše aktuální editovanou hodnotu
  			}
	...
	for(unsigned int i=1;i<=E->mGrid->RowCount-1;i++)
	{
		...
		for(unsigned int j=1;j<=E->mGrid->ColCount-1;j++)
		{
			...
			if((E->eID!=300 && E->eID!=200 || E->eID==300 && i!=1 || E->eID==200 && i!=1) && E->mGrid->Cells[j][i].Type!=E->mGrid->EDIT&&E->mGrid->Cells[j][i].Type!=E->mGrid->COMBO)
	  	{
			...
			if((E->eID==300 || E->eID==200) && i==1)E->mGrid->Cells[j][i].Font->Color=clFontLeft;
		}
	}
	//sloučení buněk hlavičky
	if(E->eID==300 || E->eID==200)E->mGrid->MergeCells(0,0,2,0);
...
void TForm1::zmena_editovanych_bunek(Cvektory::TElement *E)
{
    ...
    //kontrola zda jsou buňky na draw
		if(!C1->Enabled)
		{
			if(E->mGrid->Cells[1][3].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,1,3);E->mGrid->Cells[1][3].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[1][3].Font->Color=(TColor)RGB(128,128,128);}
			if(E->mGrid->Cells[1][5].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,1,5);E->mGrid->Cells[1][5].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[1][5].Font->Color=(TColor)RGB(128,128,128);}
			if(E->mGrid->Cells[1][6].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,1,6);E->mGrid->Cells[1][6].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[1][6].Font->Color=(TColor)RGB(128,128,128);}
		}
		if(!C2->Enabled)
		{
			if(E->mGrid->Cells[2][3].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,2,3);E->mGrid->Cells[2][3].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[2][3].Font->Color=(TColor)RGB(128,128,128);}
			if(E->mGrid->Cells[2][5].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,2,5);E->mGrid->Cells[2][5].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[2][5].Font->Color=(TColor)RGB(128,128,128);}
			if(E->mGrid->Cells[2][6].Type==E->mGrid->EDIT){mGrid_komponenta_na_draw(E->mGrid,2,6);E->mGrid->Cells[2][6].Background->Color=(TColor)RGB(240,240,240);E->mGrid->Cells[2][6].Font->Color=(TColor)RGB(128,128,128);}
		}
		//kontrola zda můžu editovat pohon
		if(C1->Enabled)
		{
...
//		}
		...
		case 300://výhybky
		{
			...
			E->mGrid->Cells[0][4].Text="Rozmezí rychlosti "+rychlost;
			E->mGrid->Cells[0][5].Text=ls->Strings[209]+" "+R;//"Rozteč palce"
			E->mGrid->Cells[0][6].Text=ls->Strings[210];//"Násobek rozteče palců"
			E->mGrid->Cells[0][7].Text=ls->Strings[211]+" "+Rz;//"Rozteč jigů"
			E->mGrid->Cells[0][8].Text=ls->Strings[212]+" "+Rz;//"Mezera mezi podvozky"
			E->mGrid->Cells[0][9].Text=ls->Strings[213]+" "+Rz;//"Mezera mezi jigy 0°"
			E->mGrid->Cells[0][10].Text=ls->Strings[214]+" "+Rz;//"Mezera mezi jigy 90°"
			E->mGrid->Cells[0][11].Text="max WT "+cas;
			//nastavování hodnot druhého sloupce
			E->mGrid->Cells[1][2].Type=E->mGrid->COMBO;
			if(E->pohon!=NULL)
			{
				E->mGrid->Cells[1][3].Type=E->mGrid->EDIT;E->mGrid->Cells[1][3].Text=m.round2double(outaRD(E->pohon->aRD),3);
				E->mGrid->Cells[1][4].Text=AnsiString(m.round2double(outaRD(E->pohon->rychlost_od),3))+" - "+AnsiString(m.round2double(outaRD(E->pohon->rychlost_do),3));
				E->mGrid->Cells[1][5].Type=E->mGrid->EDIT;E->mGrid->Cells[1][5].Text=m.round2double(outR(E->pohon->roztec),3);
				E->mGrid->Cells[1][6].Type=E->mGrid->EDIT;E->mGrid->Cells[1][6].Text=m.round2double(E->pohon->Rx,3);
				E->mGrid->Cells[1][7].Text=m.round2double(outRz(E->pohon->Rz),3);
				E->mGrid->Cells[1][8].Text=m.round2double(outRz(m.mezera(0,E->pohon->Rz,0)),3);
				E->mGrid->Cells[1][9].Text=m.round2double(outRz(m.mezera(0,E->pohon->Rz,1)),3);
				E->mGrid->Cells[1][10].Text=m.round2double(outRz(m.mezera(90,E->pohon->Rz,1)),3);
			}
			else
			{
			...
			if(E->eID==300)//pro výhybku
			{
				...
				{
					E->mGrid->Cells[2][3].Type=E->mGrid->EDIT;E->mGrid->Cells[2][3].Text=m.round2double(outaRD(E->dalsi2->pohon->aRD),3);
					E->mGrid->Cells[2][4].Text=AnsiString(m.round2double(outaRD(E->dalsi2->pohon->rychlost_od),3))+" - "+AnsiString(m.round2double(outaRD(E->dalsi2->pohon->rychlost_do),3));
					E->mGrid->Cells[2][5].Type=E->mGrid->EDIT;E->mGrid->Cells[2][5].Text=m.round2double(outR(E->dalsi2->pohon->roztec),3);
					E->mGrid->Cells[2][6].Type=E->mGrid->EDIT;E->mGrid->Cells[2][6].Text=m.round2double(E->dalsi2->pohon->Rx,3);
					E->mGrid->Cells[2][7].Text=m.round2double(outRz(E->dalsi2->pohon->Rz),3);
					E->mGrid->Cells[2][8].Text=m.round2double(outRz(m.mezera(0,E->dalsi2->pohon->Rz,0)),3);
					E->mGrid->Cells[2][9].Text=m.round2double(outRz(m.mezera(0,E->dalsi2->pohon->Rz,1)),3);
					E->mGrid->Cells[2][10].Text=m.round2double(outRz(m.mezera(90,E->dalsi2->pohon->Rz,1)),3);
				}
				else
				{
			...
			else//pro PM
			{
				...
				if(e_pom->pohon!=NULL)
				{
					E->mGrid->Cells[2][3].Type=E->mGrid->EDIT;E->mGrid->Cells[2][3].Text=m.round2double(outaRD(e_pom->pohon->aRD),3);
					E->mGrid->Cells[2][4].Text=AnsiString(m.round2double(outaRD(e_pom->pohon->rychlost_od),3))+" - "+AnsiString(m.round2double(outaRD(e_pom->pohon->rychlost_do),3));
					E->mGrid->Cells[2][5].Type=E->mGrid->EDIT;E->mGrid->Cells[2][5].Text=m.round2double(outR(e_pom->pohon->roztec),3);
					E->mGrid->Cells[2][6].Type=E->mGrid->EDIT;E->mGrid->Cells[2][6].Text=m.round2double(e_pom->pohon->Rx,3);
					E->mGrid->Cells[2][7].Text=m.round2double(outRz(e_pom->pohon->Rz),3);
					E->mGrid->Cells[2][8].Text=m.round2double(outRz(m.mezera(0,e_pom->pohon->Rz,0)),3);
					E->mGrid->Cells[2][9].Text=m.round2double(outRz(m.mezera(0,e_pom->pohon->Rz,1)),3);
					E->mGrid->Cells[2][10].Text=m.round2double(outRz(m.mezera(90,e_pom->pohon->Rz,1)),3);
				}
				else
				{
					E->mGrid->Cells[2][3].Text=0;
			...
			E->mGrid->Cells[2][11].Text=outPT(E->WT);
			//nastavení šířek
			E->mGrid->SetColumnAutoFit(-4);
			E->mGrid->Columns[0].Width=210;
			E->mGrid->Columns[1].Width=145;
			E->mGrid->Columns[2].Width=145;
			//nastavení exButtonu, skrývání řádku max.WT Stop
			E->mGrid->exBUTTONVisible=true;
			E->mGrid->exBUTTON->ShowHint=true;
			E->mGrid->exBUTTON->Hint=ls->Strings[231];//"Rozšířené položky";
			//hinty
			E->mGrid->Cells[0][11].Hint=ls->Strings[229];//"maximální možná doba čekání na palec";
...
//		}
		...
		case 300://výhybka
		{
			...
			if(E->pohon!=NULL)
			{
				E->mGrid->Cells[1][3].Text=m.round2double(outaRD(E->pohon->aRD),3);
				E->mGrid->Cells[1][4].Text=AnsiString(m.round2double(outaRD(E->pohon->rychlost_od),3))+" - "+AnsiString(m.round2double(outaRD(E->pohon->rychlost_do),3));
				E->mGrid->Cells[1][5].Text=m.round2double(outR(E->pohon->roztec),3);
				E->mGrid->Cells[1][6].Text=m.round2double(E->pohon->Rx,3);
				E->mGrid->Cells[1][7].Text=m.round2double(outRz(E->pohon->Rz),3);
				E->mGrid->Cells[1][8].Text=m.round2double(outRz(m.mezera(0,E->pohon->Rz,0)),3);
				E->mGrid->Cells[1][9].Text=m.round2double(outRz(m.mezera(0,E->pohon->Rz,1)),3);
				E->mGrid->Cells[1][10].Text=m.round2double(outRz(m.mezera(90,E->pohon->Rz,1)),3);
...
void TForm1::redesign_element()
{
  log(__func__);//logování
	...
	switch (pom_element->eID)
	{
		...
		case 300://výhybka
		{
			if(JID==103 || JID==104)zrychlost=true;
			if(JID==111)zcas=true;
			if(JID==105)zR=true;
			if(JID>=107 && JID<=110)zRz=true;
			break;
    }
	}
...
//		}
		...
		case 300://výhybka
		{
			...
			{
				{
					E->mGrid->Cells[2][3].Text=m.round2double(outaRD(E->dalsi2->pohon->aRD),3);
					E->mGrid->Cells[2][4].Text=AnsiString(m.round2double(outaRD(E->dalsi2->pohon->rychlost_od),3))+" - "+AnsiString(m.round2double(outaRD(E->dalsi2->pohon->rychlost_do),3));
					E->mGrid->Cells[2][5].Text=m.round2double(outR(E->dalsi2->pohon->roztec),3);
					E->mGrid->Cells[2][6].Text=m.round2double(E->dalsi2->pohon->Rx,3);
					E->mGrid->Cells[2][7].Text=m.round2double(outRz(E->dalsi2->pohon->Rz),3);
					E->mGrid->Cells[2][8].Text=m.round2double(outRz(m.mezera(0,E->dalsi2->pohon->Rz,0)),3);
					E->mGrid->Cells[2][9].Text=m.round2double(outRz(m.mezera(0,E->dalsi2->pohon->Rz,1)),3);
					E->mGrid->Cells[2][10].Text=m.round2double(outRz(m.mezera(90,E->dalsi2->pohon->Rz,1)),3);
			...
			if(E->eID==200)//pro PM
			{
				...
				if(e_pom->pohon!=NULL)
				{
					E->mGrid->Cells[2][3].Text=m.round2double(outaRD(e_pom->pohon->aRD),3);
					E->mGrid->Cells[2][4].Text=AnsiString(m.round2double(outaRD(e_pom->pohon->rychlost_od),3))+" - "+AnsiString(m.round2double(outaRD(e_pom->pohon->rychlost_do),3));
					E->mGrid->Cells[2][5].Text=m.round2double(outR(e_pom->pohon->roztec),3);
					E->mGrid->Cells[2][6].Text=m.round2double(e_pom->pohon->Rx,3);
					E->mGrid->Cells[2][7].Text=m.round2double(outRz(e_pom->pohon->Rz),3);
					E->mGrid->Cells[2][8].Text=m.round2double(outRz(m.mezera(0,e_pom->pohon->Rz,0)),3);
					E->mGrid->Cells[2][9].Text=m.round2double(outRz(m.mezera(0,e_pom->pohon->Rz,1)),3);
					E->mGrid->Cells[2][10].Text=m.round2double(outRz(m.mezera(90,e_pom->pohon->Rz,1)),3);
...
void TForm1::NP_input()
{
   ...
   if(zmena_zoom)
   {
	...
	if(OBJEKT_akt->element!=NULL)
	{
		...
		E=NULL;delete E;
		if(odmazan_pohon)vlozit_predavaci_misto_aktualizuj_WT();
	}
	mGrid_on_mGrid();
	if(OBJEKT_akt->pohon!=NULL)aktualizace_ComboPohon();//aktualizace comba pokud je pohon přiřazen
...
//	napln_comba_mGridu(OBJEKT_akt->element);
  ...
  log(__func__);//logování
    ...
    //pokud je kurzor vykreslen překresli ho
		...
		if(index_kurzoru<=-11&&OBJEKT_akt->id!=3&&Akce!=GEOMETRIE&&Akce!=GEOMETRIE_LIGHT)//editace hodnot kót elementů
		{
			...
			vlozit_predavaci_misto_aktualizuj_WT();//kontrola zda nedošlo k překryvu PM
		}
...
void TForm1::change_languagein_mGrid()
{
    ...
    ////aktualizace tabulek elementů
		if(OBJEKT_akt!=NULL)//pro jistotu
		{
			Cvektory::TElement *E=OBJEKT_akt->element;
