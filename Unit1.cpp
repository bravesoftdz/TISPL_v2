			...
			if(Button==mbLeft/* && MOD!=REZERVY*/)//je stisknuto levé tlačítko myši
			{
					...
					if(Akce==NIC && posun_objektu==false && funkcni_klavesa==0)//pokud není aktivovaná jiná akce
					{
						if(MOD==NAHLED && pom_temp!=NULL)//TABULKA či ELEMENT
						{
								...
								if(JID==7||JID==8||JID==9){design_tab_pohon(1);REFRESH();}
						}
						else
						{
...
void TForm1::design_tab_pohon(int index)
{
	...
	switch (index)
	{
   	...
   	case 0:
		{
   		...
   		//vytvoření tabulky
			if(pom_temp->elementy!=NULL/*&&pom_temp->pohon!=NULL*/)
			{
      ...
      //hlavička
   	...
   	case 1:///////////Změna jednotek
   	{
		...
		{
			int index=PmG->getCombo(1,1)->ItemIndex;
			...
			if(index==0&&PmG->RowCount!=2)
			{
				if(PmG->RowCount==6) {PmG->DeleteRow(5,false);PmG->DeleteRow(4,false);PmG->DeleteRow(3,false);PmG->DeleteRow(2,false);}
				else {PmG->DeleteRow(3,false);PmG->DeleteRow(2,false);}
			}
		}break;
	...
	if(PmG->RowCount==4)
	{
		...
		PmG->Cells[1][2].Text=outaRD(pom_temp->pohon->aRD);
	...
	for(int i=1;i<=ms.MyToDouble(PmG->RowCount-1);i++)
	{
		...
		else if(PmG->Cells[1][i].Type!=PmG->COMBO)
		{
...
void TForm1::akt_tabulek (Cvektory::TElement *E,AnsiString LO,AnsiString delka_otoce,AnsiString cas,short sirka_0,short sirka_1,short sirka_2,short sirka_3,short sirka_4,short sirka_56,short sirka_cisla)
{
	switch(E->eID)
	{
		...
		case 5://otoč pasivní
		{
			...
			E->mGrid->Cells[1][2].Text=outPT(E->PTotoc);
//			E->mGrid->getEdit(1,1)->Text=99; //test přepisování při focus na edit .. funguje takto
			E->mGrid->Columns[0].Width=sirka_56;
			E->mGrid->Columns[1].Width=sirka_cisla;
			break;
		}
          ...
          TRect &Rect, TGridDrawState State)
{
  ...
  AnsiString label2;
    ...
    if(n==2){ label1= "aktivní"; label2=""; }
		if(pom_temp->id==3)
		{
				 d.vykresli_otoc(C,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P - 15-odsazeni,label1,label2,n+4,0,0,1);
		}
		else d.vykresli_otoc(C,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P - 15-odsazeni,label1,label2,n+4,0,0,-1);
	}

	if((EID==1||EID==3))
	{
		d.vykresli_otoc(C,(Rect.Right*Z-Rect.Left*Z)/2+((2)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(1/2.0)-1)*H+P - 15-odsazeni,"pasivní","",1+4,0,0,1);
		d.vykresli_otoc(C,(Rect.Right*Z-Rect.Left*Z)/2+((3)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(2/2.0)-1)*H+P - 15-odsazeni,"aktivní","",2+4,0,0,-1);
	}
	if((EID==2||EID==4))
	{
		d.vykresli_otoc(C,(Rect.Right*Z-Rect.Left*Z)/2+((2)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(1/2.0)-1)*H+P - 15-odsazeni,"pasivní","",1+4,0,0,-1);
...
//	unsigned short obdelnik_okrajX=10*Z;unsigned short obdelnik_okrajY=5*Z;
	...
	if(pom_temp->id==3)
	{
					d.vykresli_stopku(C,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P + 2-odsazeni,"STOP","");
	} else  d.vykresli_stopku(C,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P + 2-odsazeni,"STOP","",0,0,-1);
    ...
    if(n==1)
    {
     ...
     label2="";
		...
		if(n==2)
		{
		 label1= "oblouky";
		 label2="";
		 if(pom->id!=3) d.vykresli_ikonu_oblouku(C,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P + 20-odsazeni,label1);
...
void TForm1::vykresli_kurzor(int index)
{
	...
	if(index<=-11)
	{
      Canvas->Pen->Color=clGray;
			Canvas->Pen->Width=1.5;
			Canvas->MoveTo(pom_element_temp->kota_oblast.rect1.right+1,pom_element_temp->kota_oblast.rect1.top);
			Canvas->LineTo(pom_element_temp->kota_oblast.rect1.right+1,pom_element_temp->kota_oblast.rect1.bottom);
			stav_kurzoru=!stav_kurzoru;
  }
}
