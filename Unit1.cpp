  ...
  if(Screen->Cursor!=standard)kurzor(standard);//ošetření proti zůstávání kurzoru
	...
	switch(MOD)
	{
		...
		case NAHLED:
		{
			...
			if (pom_element!=NULL/* && Akce!=GEOMETRIE*/ && pom_element->eID!=200)//Pokud bylo kliknuto na element + ošetření pom_element je používan i při tvorbě geometrie + nejedná se o předávací místo
			{
...
void TForm1::ESC()
{
	...
	switch(Akce)
	{
		...
		case NIC:if(pom_temp!=NULL && !editace_textu)KonecClick(this);break;
	}
	pom_vyhybka=NULL;
	proces_pom=NULL;
	kurzor(standard);
...
void TForm1::vytvoreni_tab_knihovna()
{
	...
	mGrid_knihovna->Cells[1][11].ImageIndex=13;
	...
	{
		if(i==7||i==15||i==14||i==13||i==12)odsazeni-=5;
		//pro první sloupec
...
void TForm1::popisky_knihovna_nahled(bool knihovna)
{
	...
	if(knihovna)//popisky pro knihovnu
	{
		...
		scGPLabel_geometrie->Caption="Lakování";scGPLabel_geometrie->Top=314-5;
		scGPLabel_poznamky->Caption="Ostatní";scGPLabel_poznamky->Top=405-5;
  }
	else
	{
...
{
	log(__func__);//logování
	if(Akce!=NIC)ESC();//zruší případnou rozdělanou akci
...
{
  log(__func__);//logování
  if(Akce!=NIC)ESC();//zruší případnou rozdělanou akci  - způsobuje problém při zavírání splitview
  scSplitView_OPTIONS->Opened = !scSplitView_OPTIONS->Opened;
