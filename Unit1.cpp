		...
		if(!dblClick)//v případě, že se nejedná o double click
		{
			if(Button==mbLeft)//je stisknuto levé tlačítko myši
			{
				...
				switch(Akce)
				{
					...
					{
						if(JID==-11&&!editace_textu){nastav_focus();TimerKurzor->Enabled=true;editace_textu=true;stav_kurzoru=false;index_kurzoru=JID;pom_element_temp=pom_element;editovany_text=pom_element->geo.delka;Memo(editovany_text);if((DKunit==2||DKunit==3)&&pom_element->pohon!=NULL)editovany_text=editovany_text/pom_element->pohon->aRD;editovany_text=outDK(m.round2double(ms.MyToDouble(editovany_text),3));}//editace textu
						if(JID==13){Akce_temp=OFFSET_KOTY;minule_souradnice_kurzoru=vychozi_souradnice_kurzoru;}//offset kót
					}break;
        ...
        //začátek a konec měření
          ...
          //uložení parametrů geometrie z úseku, stoupání, typ atd.
			  	...
			  	else
			  	{
					...
					if(E->geo.X2==E->geo.X3 && d.v.MAG_LASO->predchozi->n==0)TIP="Přichyceno na "+E->name;
					E=NULL;delete E;
				}
...
void TForm1::zmena_editovanych_bunek(Cvektory::TElement *E)
{
    ...
    //kontrola zda jsou buňky na draw
		...
		rezim=-1;
		if(e_pom->pohon!=NULL)rezim=e_pom->pohon->rezim;
		if(C2!=NULL && C2->Enabled)
		{
			//pokud pohon exituje a není používaný
...
//	E=NULL;delete E;
//  Memo("hotovo");
	Memo(scGPButton_bug_report->Top); //=957
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
	...
	//if(pom->id!=3)//nevykreslovat nic v POW
	{
  	...
  	for(unsigned short n=1;n<=pocet_elementu;n++)
  	{
  		if(n==1)
  		{
			 ...
			 label2="";
			 if(d.v.ZAKAZKA_akt==NULL || (d.v.ZAKAZKA_akt!=NULL && d.v.ZAKAZKA_akt->n==0))d.vykresli_ikonu_linie(C,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P + 20-odsazeni,label1);
  		...
  		if(n==2)
  		{
				...
				if(OBJEKT_akt->pohon!=NULL && (d.v.ZAKAZKA_akt==NULL || d.v.ZAKAZKA_akt!=NULL && d.v.ZAKAZKA_akt->n==0))d.vykresli_predavaci_misto(C,NULL,(Rect.Right*Z-Rect.Left*Z)/2+((n+1)%2)*W,(Rect.Bottom*Z-Rect.Top*Z)/2+(ceil(n/2.0)-1)*H+P,label1,0);
...
void TForm1::vykresli_kurzor(int index)
{
  log(__func__);//logování
	...
	switch ((index))//index=JID, kde a jaký kurzor vykreslit
	{
		...
		{
      if(OBJEKT_akt->id!=3)//kóty elementů
    	{
    		Canvas->Pen->Color=clGray;
    		Canvas->Pen->Width=1.5;
    		if(index!=-101)
...
//						if(s<=buf)
//						{
//							cas+=m.V2WT(ceil((buf-s)/d.v.PP.delka_podvozek),d.v.PP.TT);//připočítání WT na aktuálním vozíku
//							cas_pom+=m.V2WT(ceil((buf-s)/d.v.PP.delka_podvozek),d.v.PP.TT);
...
//											cas_pom+=m.V2WT(ceil((dl-buf)/d.v.PP.delka_podvozek),d.v.PP.TT);
...
//							cas+=s/CE->pohon->aRD;//výpočet času přejezdu
//							if(C->n==1)//výpočet času pro první sgment
//							{
...
//									{
//										cas+=m.V2WT(ceil((buf-s)/d.v.PP.delka_podvozek),d.v.PP.TT);//připočítání WT na aktuálním vozíku
//										cas-=(buf-s)/CE->pohon->aRD;//zmenšení délky jen na délku pojezdu
//						  		}
//								}
//							}
