  ...
  //dočasné vypnutí záložky editace
	...
	for(unsigned short i=0;i<=425;i++)
	{
      ...
      case 419:text="čekání";break;
			...
			case 424:text="Nepodařilo se načíst podklad, zkontrolujte jeho existenci, nebo proveďte nové načtení.";break;
...
void TForm1::startUP()
{
  ...
  if(!Form_zpravy->closing && !Form_zpravy->Showing && !PopUPmenu->Showing && !PopUPmenu->closing)//pozn. dole ještě větev else if(PopUPmenu->Showing || PopUPmenu->closing)PopUPmenu->Close();//pokud je spuštěné pop-up menu, tak ho vypne
    ...
    if(volat_parametry_linky)Button_dopravnik_parametryClick(this);//zavolání formáláře pro prvotní vyplnění či potvzení hodnot parametrů linky
		...
		if(d.v.OBJEKTY->dalsi==NULL)
    {
		...
		if(d.v.PP.raster.filename!="" && !FileExists(d.v.PP.raster.filename) && mrYes==MB(ls->Strings[424],MB_YESNO))scButton_nacist_podkladClick(this);//"Nepodařilo se načíst podklad, zkontrolujte jeho existenci, nebo proveďte nové načtení."
	}
...
void TForm1::smaz_edit(bool refresh)
{
  log(__func__);//logování
	...
	{
		pom_element_temp=pom_temp->elementy->dalsi;
		...
		if(!dblClick)//v případě, že se nejedná o double click
		{
            ...
            //nové JID pro objekt
					...
					else
					{
						...
						{
							if(JID==3&&!d.v.PP.zamek_layoutu){Akce=MOVE;kurzor(posun_l);minule_souradnice_kurzoru=TPoint(X,Y);predchozi_souradnice_kurzoru=m.L2P(pom->elementy->dalsi->geo.X1,pom->elementy->dalsi->geo.Y1);}
		...
		case GEOMETRIE:
		{
			if(Akce_temp==NIC)
			{
		  	...
		  	if(stisknute_leve_tlacitko_mysi){pan_map(Canvas,X,Y);kurzor(pan_move);}else if(Screen->Cursor==pan_move)kurzor(standard);
				...
				if(pom->dalsi!=NULL && e_poslendi->geo.X4!=pom->dalsi->elementy->dalsi->geo.X1 && e_poslendi->geo.X4!=pom->dalsi->elementy->dalsi->geo.X1)
		  	{
		  		//nastavení pera
...
void TForm1::vykresli_spojinici_EmGrid(TCanvas *Canv,Cvektory::TElement *E)
{
	...
	if(pom_temp!=NULL && pom_temp->zobrazit_mGrid && E!=NULL && E->mGrid!=NULL && d.v.vrat_druh_elementu(E)!=-1)
	{
...
void TForm1::setJobIDOnMouseMove(int X, int Y)
{
	...
	if(MOD==SCHEMA)//pro schéma, změna kurzorů pro body a úsečky
	{
		...
		if(puvJID!=JID || pom_bod_puv!=pom_bod)//pokud došlo ke změně JID, nebo změně bodu bez změny JID, jinak nemá smysl řešit
		{
			...
			if(JID==0||(JID==3&&!d.v.PP.zamek_layoutu))kurzor(posun_ind);//posun bodu
...
{																																																																								 																									//"Objekty nenavazují, automaticky posunout následující objekt pro zachování návaznosti?"
	...
	if(pom->dalsi!=NULL && !(e_posledni->geo.X4==pom->dalsi->elementy->dalsi->geo.X1 && e_posledni->geo.Y4==pom->dalsi->elementy->dalsi->geo.Y1) && mrYes==MB(ls->Strings[328],MB_YESNO,true))
	{
...
//				E->short_name=E->name.SubString(1,3)+AnsiString(nTyp);
	...
	if(d.v.OBJEKTY->dalsi!=NULL && d.v.OBJEKTY->predchozi->n>=3)
	{
		...
		//////////Vkládání předávacího místa
		if(O->elementy->dalsi->pohon!=NULL && e_posledni->pohon==NULL || O->elementy->dalsi->pohon==NULL && e_posledni->pohon!=NULL || O->elementy->dalsi->pohon!=NULL && e_posledni->pohon!=NULL && O->elementy->dalsi->pohon->n!=e_posledni->pohon->n)
		{
		...
		{
			if(O->elementy->dalsi->pohon!=NULL)e_posledni->WT=m.cekani_na_palec(0,O->elementy->dalsi->pohon->roztec,O->elementy->dalsi->pohon->aRD,3);
			if(pom_temp!=NULL && pom_temp->n==O_posledni->n)e_posledni->mGrid->Refresh();
		}
		//////////Mazání nepotřebného předávacího místa (změna na zarážku)
		if(e_posledni->eID==200 && (O->elementy->dalsi->pohon==NULL && e_posledni->pohon==NULL || O->elementy->dalsi->pohon!=NULL && e_posledni->pohon!=NULL && O->elementy->dalsi->pohon->n==e_posledni->pohon->n))
		{
			//vynulování WT
			e_posledni->WT=0;
			//změna na zarážku
...
bool TForm1::el_vkabine(double X,double Y,int element_id,short orientace_el,Cvektory::TElement *E,double tolerance_x,double tolerance_y)
{
	...
	x2=e_posledni->geo.X4;y2=e_posledni->geo.Y4;
	e_posledni=NULL;delete e_posledni;
...
{        
	...
	if(pom_temp!=NULL && E!=NULL && d.v.vrat_druh_elementu(E)!=-1)
	{
		...
		if(!prekryti)
		{
			...
			switch((int)E->geo.orientace)
			{
				case 0:if(dalsi && el1.bottom>m.L2Py(pom_temp->elementy->dalsi->geo.Y1) || predchozi && m.L2Py(e_posledni->geo.Y4)>el1.top)prekryti=true;break;
				case 90:if(dalsi && el1.left<m.L2Px(pom_temp->elementy->dalsi->geo.X1) || predchozi && m.L2Px(e_posledni->geo.X4)<el1.right)prekryti=true;break;
				case 180:if(dalsi && el1.top<m.L2Py(pom_temp->elementy->dalsi->geo.Y1) || predchozi && m.L2Py(e_posledni->geo.Y4)<el1.bottom)prekryti=true;break;
...
void TForm1::aktualizace_RT()
{
	...
	if(pom_temp!=NULL)
	{
		...
		while(E!=NULL)
		{
			...
			if(E->eID==0)
			{
				E->max_pocet_voziku=max_voziku(E);
				try
				{
			...
			//aktualizace RT
			if(E->eID%2==0 && E->eID!=100 && E->eID!=200 && E->eID!=MaxInt)d.v.reserve_time(E);
			E=E->dalsi;
		}
		delete E;E=NULL;
	}
...
void TForm1::nahled_ulozit (bool duvod_ulozit)
{
	...
	if(duvod_ulozit_nahled){if(stisknute_leve_tlacitko_mysi)duvod_validovat=1;else duvod_validovat=2;}//pokud je důvod validovat, tj. až po dokončení operace=1, aby neustále neproblikávalo i během posunu elementů
}
//---------------------------------------------------------------------------
...
{
	log(__func__);//logování
	double puv_RT=E->RT.y;//RT, obsahující validaci
	//extrakce pouze RT
	if(E->RT.y>1000000)E->RT.y-=1000000;
	if(E->RT.y<-1000000)E->RT.y+=1000000;
	switch(E->eID)
	{
		case 0://stop stanice
		{
			...
			E->mGrid->Cells[0][2].Text="RT "+cas;
			if(E->RT.y>0 && E->akt_pocet_voziku>1){E->mGrid->Cells[1][2].Text="OK";E->mGrid->Cells[1][2].Hint=outPT(E->RT.y);E->mGrid->Cells[1][2].ShowHint=true;}
			else {E->mGrid->Cells[1][2].Text=outPT(E->RT.y);E->mGrid->Cells[1][2].Hint="";E->mGrid->Cells[1][2].ShowHint=false;}
			E->mGrid->Cells[0][3].Text=popisek+" "+cas;
			E->mGrid->Cells[1][3].Text=outPT(E->WTstop);
			E->mGrid->Cells[0][4].Text=ls->Strings[224]+" "+cas;//"WT palec "
		...
		case 2://robot se stop stanicí
		{
			...
			E->mGrid->Cells[0][1].Text="PT "+cas;
			E->mGrid->Cells[1][1].Type=E->mGrid->EDIT;E->mGrid->Cells[1][1].Text=outPT(E->PT1);
			E->mGrid->Cells[0][2].Text="RT "+cas;E->mGrid->Cells[1][2].Text=outPT(E->RT.y);
...
//			E->mGrid->DefaultCell.isLink->Color=clFontRight;//přiřazení barvy fontu
			...
			E->mGrid->Cells[1][4].Type=E->mGrid->EDIT;E->mGrid->Cells[1][4].Text=outPT(E->PT2);
			E->mGrid->Cells[0][5].Text="RT "+cas;
			E->mGrid->Cells[1][5].Text=outPT(E->RT.y);
			E->mGrid->Cells[0][6].Text="WT "+cas;
		...
		case 6://otoč aktivní (resp. otoč se stop stanicí)
		{
			...
			E->mGrid->Cells[1][2].Type=E->mGrid->EDIT;E->mGrid->Cells[1][2].Text=outPT(E->PTotoc);
			E->mGrid->Cells[0][3].Text="RT "+cas;
			E->mGrid->Cells[1][3].Text=outPT(E->RT.y);
			//automatické nastavení sířky sloupců podle použitých jednotek
			E->mGrid->SetColumnAutoFit(-4);
			E->mGrid->Columns[0].Width=sirka_56;
			E->mGrid->Columns[1].Width=sirka_cisla;
	...
	E->RT.y=puv_RT;//navrácení validace k RT hodnotě
}
//---------------------------------------------------------------------------
...
void TForm1::akt_tabulek (Cvektory::TElement *E,AnsiString LO,AnsiString delka_otoce,AnsiString cas,short sirka_0,short sirka_1,short sirka_2,short sirka_3,short sirka_4,short sirka_56,short sirka_cisla)
{
	...
	switch(E->eID)
	{
		case 0://stop stanice
		{
			...
			E->mGrid->Cells[0][2].Text="RT "+cas;
			E->mGrid->Cells[1][2].Text=m.round2double(outPT(E->RT.y),3);
			E->mGrid->Cells[0][3].Text=popisek+" "+cas;
		...
		case 4:case 10:case 14:case 18:case 104:case 108://robot s aktivní otočí (resp. s otočí a stop stanicí)
		{
			...
			E->mGrid->Cells[1][5].Text=m.round2double(outPT(E->RT.y),3);
		...
		case 6://otoč aktivní (resp. otoč se stop stanicí)
		{
			...
			E->mGrid->Cells[0][3].Text="RT "+cas;
			E->mGrid->Cells[1][3].Text=m.round2double(outPT(E->RT.y),3);
			E->mGrid->Columns[0].Width=sirka_56;
			E->mGrid->Columns[1].Width=sirka_cisla;
			break;
		}
...
//	while(O!=NULL)
//	{
		Cvektory::TElement *E=pom_temp->elementy->predchozi->predchozi; Memo3->Clear();
//  	while(E!=NULL)
//  	{
