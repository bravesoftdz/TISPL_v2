...
class Cmy
{
  ...
  long px_offset(unsigned int X,unsigned int Y,long Distance);//vráti souøadnice pixelù posunuté o Distance METRù
	...
	void frameRect(TRect Rect,TColor color=clWebOrange,short width=1);//nakresli na libovolném místì na monitoru obdelník dle zadaných souøadnic, je možné nastavit barvu a šíøku èáry tohoto obdelníku
