...
TmGrid::TmGrid(TForm *Owner)
{
	...
	*Note->Font=*DefaultCell.Font;
	...
	if(ID!=9999)Note->Text="Text vıpisu poznámky pod èarou a nìjakı další abcdefgeijasdfads dafs";
}
//---------------------------------------------------------------------------
...
unsigned long TmGrid::getHeight()
{
	...
	if(Note->Text!="" && ColCount>0 && RowCount>0)
	{
		...
		if(Form->Canvas->TextWidth(Note->Text)>getWidth())Offset_Note=2;else Offset_Note=1; //pokud je text delší musí se zobrazit 2 øádky, Offset_Note - jenom "zneuívám"
