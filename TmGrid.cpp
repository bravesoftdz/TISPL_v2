...
void TmGrid::SetComponents(TCanvas *Canv,TRect R,TRect Rt,unsigned long X,unsigned long Y,TCells &Cell)
{
	switch(Cell.Type)
	{
		...
		{   //Cell.Text=getTag(X,Y);//provizorní vıpis pokud chci oèíslovat jednotlivé buòky
			...
			//zarovnání
			//samotnı vıpis
			long L=Rt.Left,T=Rt.Top;              //zajimavı workaround - pøíèinì nerozumím  - pravdìpodobnì chyba  s pozicováním bmp v rámci celé tabulky
			int W=getWidthHeightText(Cell).X*Zoom;//if(AntiAliasing_text && (Cell.Font->Orientation!=0 || Cell.Align!=aNO))W/=1.3;// - provizornì odstaveno - chová se to bez toho lépe u neslouèenıch bunìk na støed, ale zase mùe chybìt nìkdì jinde
			...
			short WA=0;if(AntiAliasing_text && (Cell.Font->Orientation!=0 || Cell.Align!=aNO))WA=1;//zajimavı workaround - pøíèinì nerozumím (proè Left*2 tomu patøiènì pomùe)
			switch(Cell.Align)
			{               //zajimavı workaround - pravdìpodobnì chyba s pozicováním bmp v rámci celé tabulky proto tam byl
			...
			}                                    //*2 zajimavı workaround   - pravdìpodobnì chyba s pozicováním bmp v rámci celé tabulky proto tam byl
			//if(AntiAliasing_text)Canv->TextOut(L-Left*2,T-Top,Cell.Text);  jednalo se chybu hıbalo se s celou tabulkou chybnì Left a Top, ale pøi AA je souèástí BMP která se a vıslednì pozicuje  //zde není Left celé tabulky, protoe se pozicije na pozici levého horního rohu tabulky celá bmp, take zde musí bıt pouze souøadnice v rámci tabulku, nikoliv absolutnì v celém formu
			//else
			 ...
			 case CENTER:
			 {
				 switch(RefCell.Type)
				 {
						...
						case NUMERIC:
						{
							...
							N->Value=F->ms.MyToDouble(RefCell.Text);//bere a z poslední buòky sluèované oblasti
							N=NULL;delete N;
						}break;
						case LABEL:
						{
