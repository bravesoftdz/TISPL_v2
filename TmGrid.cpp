...
void TmGrid::SetEdit(TRect R,unsigned long X,unsigned long Y,TCells &Cell)
{
	...
	//E->ShowHint=false;//toto by bylo vdy u editu na false, pokus pro dlouhı textif(Cell.Text.Length()>E->Width/(Cell.Font->Size-2))E->ShowHint=true;else //asi nepøesné
...
void __fastcall TmGrid::getTagOnKeyPress(TObject *Sender,System::WideChar &Key)
{
	if(!deleteMark)//detekce e nedochází k odstraòování mGridu, pøitom nesmí k události docházet
	{
		...
		//filtr kláves
		if(Cells[Col][Row].InputNumbersOnly && Key!=VK_BACK)//pokud je nastaveno na true a není stisknuta klávesa backspace, nelze vepsat jinou hodnotu ne èíselnou (to vèetnì reálného èísla)
		{
			...
			if(AnsiString(Key) == Separator && Cells[Col][Row].Text.Length()      == 0)Key=0;//oddìlovaè nemùe bıt na prvním místì
			if(AnsiString(Key) ==	Separator && Pos(Separator,Cells[Col][Row].Text) > 0)Key=0;//oddìlovaèù nemùe bıt více
