...
void TmGrid::SetEdit(TRect R,unsigned long X,unsigned long Y,TCells &Cell)
{
	...
	E->Options->FrameNormalColor=Cell.Background->Color;//rámeèek musí bıt stejnou barvou jakou buòka, protoe møíka je o 1px na všechny strany roztaená
...
void TmGrid::SetNumeric(TRect R,unsigned long X,unsigned long Y,TCells &Cell)
{
	...
	N->Options->FrameNormalColor=Cell.Background->Color;//rámeèek musí bıt stejnou barvou jakou buòka, protoe møíka je o 1px na všechny strany roztaená
...
void __fastcall TmGrid::getTagOnKeyPress(TObject *Sender,System::WideChar &Key)
{
	if(!deleteMark)//detekce e nedochází k odstraòování mGridu, pøitom nesmí k události docházet
	{
		...
		//filtr kláves
		if(Cells[Col][Row].InputNumbersOnly)Key=ms.numericFilter(Cells[Col][Row].Text,Key);//pokud je nastaveno na true a není stisknuta klávesa backspace, nelze vepsat jinou hodnotu ne èíselnou (to vèetnì reálného èísla)
...
//---------------------------------------------------------------------------
//zajistí zvıraznìní vnìjšího orámování buòky
...
//---------------------------------------------------------------------------
//zajistí zvıraznìní dané komponenty
