...
void TmGrid::SetColRow()
{
	...
	for(unsigned long X=0;X<ColCount;X++)//po sloupcích
	{
		if(X!=0 && (!Columns[X-1].Visible || Columns[X-1].Width==0))Columns[X-1].Left;//skrytí sloupce, pokud je pøechozí skrytı, pøevezme aktuální zpracovávanı (cyklem) pozici pøedchozího
