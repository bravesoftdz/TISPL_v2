...
Cvykresli::Cvykresli()
{
	...
	PX2SEC=30.0/10*3;//časové osy - měřítko PX na MIN, globální proměná i pro využítí výpisu ve SB v Unit1
	KrokY=30*3;//vizuální rozteč na ose Y mezi jednotlivými vozíky
	//mod_vytizenost_objektu=false;
	//NOLIEX=2;
	oY=5*3;//ofset na ose Y, 5 pouze grafická korekce
...
void Cvykresli::vykresli_vektory(TCanvas *canv, short scena)//scena 0 - vše, scena 1 - statická scéna, scena 2 - dynamická scéna (odkud je voláno), následně se vyfiltruje podle toho, jak je nastaveno
{
  ...
  vykresli_vetev_dopravniku(canv);
	...
	/////ČASOVÉ OSY
	if(v.SIM!=NULL)vykresli_casove_osy(canv,scena);
}
//---------------------------------------------------------------------------
...
void Cvykresli::vykresli_elementy(TCanvas *canv,short scena)
{
		...
		while(E!=NULL)
		{
			...
			if(scena<=2)
			{
				...
				if(pom!=NULL/* && pom->geo.delka!=0 && E->geo.delka!=0*/)
				{
					...
					if(!m.DisD(E->geo.X4,bod.x) || !m.DisD(E->geo.Y4,bod.y))
					{
				...
				{
					if(!m.DisD(E->geo.X4,v.ELEMENTY->dalsi->geo.X1) || !m.DisD(E->geo.Y4,v.ELEMENTY->dalsi->geo.Y1))
					{
						canv->MoveTo(m.L2Px(E->geo.X4),m.L2Py(E->geo.Y4));
...
void Cvykresli::vykresli_kabinu(TCanvas *canv,Cvektory::TObjekt *O,int stav,bool zobrazit_koty)
{
	...
	if(O->komora!=NULL && O->komora->predchozi->n>0)
	{
		...
		while(K->dalsi!=NULL)
		{
			if(F->OBJEKT_akt!=NULL && F->OBJEKT_akt->n==O->n && ((F->JID==-11 || F->JID==0) && F->pom_komora!=NULL && F->pom_komora->n==K->n)){clAkt=m.clIntensive(clStenaKabiny,-50);highlight_pow=true;}//highlight
			...
			if(orientace==90 || orientace==270)
			{
				...
				//highlight komory
				if(F->OBJEKT_akt!=NULL && F->OBJEKT_akt->n==O->n && ((F->JID==-11 || F->JID==0)  && F->pom_komora!=NULL && F->pom_komora->n==K->n))
				{
					double hl_X=0;
			...
			else
			{
				...
				//highlight komory
				if(F->OBJEKT_akt!=NULL && F->OBJEKT_akt->n==O->n && ((F->JID==-11 || F->JID==0)  && F->pom_komora!=NULL && F->pom_komora->n==K->n))//highlight komory
				{
					highlight_pow=true;
					double hl_Y=0;
			...
			{
				//nastavení highlight
				if(((F->JID==-11 || F->JID==0)  && F->pom_komora!=NULL && F->pom_komora->n==K->n) || (F->JID==-11 && F->pom_komora!=NULL && F->pom_komora->n==K->predchozi->n) && F->d.v.PtInKota_komory(O,F->akt_souradnice_kurzoru_PX.x,F->akt_souradnice_kurzoru_PX.y)==-1)highlight=2;
				else if((F->JID==-11 && F->pom_komora!=NULL && F->pom_komora->n==K->n) || F->JID==13)highlight=1;else highlight=0;
		...
		K=NULL;delete K;
		////poslední komora
		if(F->OBJEKT_akt!=NULL && F->OBJEKT_akt->n==O->n && ((F->JID==-11 && F->pom_komora!=NULL && F->pom_komora->n==F->OBJEKT_akt->komora->predchozi->n) || (F->JID==0  && F->pom_komora!=NULL && F->pom_komora->n==F->OBJEKT_akt->komora->predchozi->n)))clAkt=m.clIntensive(clStenaKabiny,-50);//highlight
		...
		if(orientace==90 || orientace==270)
		{
			if(F->OBJEKT_akt!=NULL && F->OBJEKT_akt->n==O->n && ((F->JID==-11 && F->pom_komora!=NULL && F->pom_komora->n==F->OBJEKT_akt->komora->predchozi->n) || (F->JID==0  && F->pom_komora!=NULL && F->pom_komora->n==F->OBJEKT_akt->komora->predchozi->n)))//highlight komory
			{
		...
		if(zobrazit_koty && F->OBJEKT_akt->n==O->n)
		{
			if((F->JID==0  && F->pom_komora!=NULL && F->pom_komora->n==O->komora->predchozi->n) || (F->JID==-11 && F->pom_komora!=NULL && (F->pom_komora->n==O->komora->predchozi->n || F->pom_komora->n==O->komora->predchozi->predchozi->n) && F->d.v.PtInKota_komory(O,F->akt_souradnice_kurzoru_PX.x,F->akt_souradnice_kurzoru_PX.y)==-1))highlight=2;
			else if((F->JID==-11 && F->pom_komora!=NULL && F->pom_komora->n==O->komora->predchozi->n) || F->JID==13)highlight=1;
			else highlight=0;
			//vykreslení kót komor
			switch((int)orientace)
			{
...
void Cvykresli::vykresli_meridlo(TCanvas *canv)
{
	...
	if(F->pom_element!=NULL && ((v.MAG_LASO->dalsi==NULL && m.Rt15(m.azimut(v.MAG_LASO->Element->geo.X1,v.MAG_LASO->Element->geo.Y1,F->akt_souradnice_kurzoru.x,F->akt_souradnice_kurzoru.y))==m.Rt15(F->pom_element->geo.orientace-F->pom_element->geo.rotacni_uhel)) || (v.MAG_LASO->dalsi!=NULL && v.MAG_LASO->sparovany==v.MAG_LASO->dalsi->Element) || (F->pom_element->geo.typ!=0 && m.T2Aarc(F->pom_element->geo.radius,m.delka(F->pom_element->geo.X1,F->pom_element->geo.Y1,F->akt_souradnice_kurzoru.x,F->akt_souradnice_kurzoru.y))>m.T2Aarc(F->pom_element->geo.radius,m.delka(F->pom_element->geo.X1,F->pom_element->geo.Y1,v.MAG_LASO->Element->geo.X1,v.MAG_LASO->Element->geo.Y1)))))
	...
	else//při prvotním kliknutí (délka měření == 0) padne měření proti trendu, řešen zde výpis při 0 mm
...
bool Cvykresli::vykresli_cit_oblasti_lasa(TCanvas *canv)
{
	...
	if(F->prichytavat_k_mrizce==1 && v.MAG_LASO->Element==NULL && v.MAG_LASO->sparovany==NULL && F->pom_element==NULL)
	{
      ...
      //kontrola začátku a konce stoupání
		...
		if(F->zobrazit_rozmisteni_voziku==1 && bod.x==MaxInt && bod.y==MaxInt)
		{
	...
	if(F->prichytavat_k_mrizce==1 && F->pom_element!=NULL)
	{
		...
		else
		{
			...
			//kontrola bodů vozíků
			Cvektory::TVozik *V=v.VOZIKY->dalsi;
			while(V!=NULL && F->zobrazit_rozmisteni_voziku==1)
			{
	  		if(m.PtInCircle(F->akt_souradnice_kurzoru.x,F->akt_souradnice_kurzoru.y,V->X,V->Y,F->velikost_citelne_oblasti_elementu))
				{
...
void Cvykresli::vykresli_meridlo_po_trendu(TCanvas *canv,bool prichyceno)
{
	...
	TPointD in,out;in.x=0;in.y=0;out.x=0;out.y=0;
	String popisek="";
  Cvektory::TPohon *pom_pohon=NULL;
	...
	if(F->pom_element!=NULL && (F->pom_element==v.MAG_LASO->predchozi->Element || F->pom_element->predchozi==v.MAG_LASO->predchozi->Element || (F->pom_element->predchozi->n==0 && F->pom_element->predchozi->predchozi==v.MAG_LASO->predchozi->Element) || (F->pom_element->predchozi->eID==301 && F->pom_element->predchozi->predchozi2==v.MAG_LASO->predchozi->Element) || F->pom_element==v.MAG_LASO->predchozi->Element || (F->pom_element->eID==301 && F->pom_element->predchozi2==v.MAG_LASO->predchozi->Element) || (F->pom_element->predchozi->eID==301 && F->pom_element->predchozi->predchozi==v.MAG_LASO->predchozi->Element)))
	{
			...
			if(C->Element!=NULL && C->Element->pohon!=NULL)
			{
				...
				if(C->n==1 && v.MAG_LASO->Element->geo.X1==C->Element->geo.X4 && v.MAG_LASO->Element->geo.Y1==C->Element->geo.Y4)
				{
					if(v.vrat_druh_elementu(C->Element)==0)
					{
						...
						in.y+=C->Element->WT;out.y+=C->Element->WT;
						//latence
						pom_pohon=NULL;if(C->Element->dalsi!=NULL)pom_pohon=C->Element->dalsi->pohon;if(C->Element->dalsi==NULL)pom_pohon=F->d.v.ELEMENTY->dalsi->pohon;//načtení pohonu do pomocného ukazatele
						if(pom_pohon!=NULL){in.y+=m.latence_mezi_stopkami(pom_pohon->aRD);out.y+=m.latence_mezi_stopkami(pom_pohon->aRD);}
						//PT robotů / operátorů
						if(C->Element->eID!=0){in.x+=C->Element->PTotoc+C->Element->data.PT1+C->Element->data.PT2;in.y+=C->Element->PTotoc+C->Element->data.PT1+C->Element->data.PT2;out.x+=C->Element->PTotoc+C->Element->data.PT1+C->Element->data.PT2;out.y+=C->Element->PTotoc+C->Element->data.PT1+C->Element->data.PT2;}
				...
				//časová data z dalších elementů
				else//if((!F->scGPCheckBox_meridlo_casy->Checked && (C->Element!=v.MAG_LASO->sparovany || (C->Element==v.MAG_LASO->sparovany && v.MAG_LASO->sparovany!=NULL && v.MAG_LASO->Element->geo.X1!=v.MAG_LASO->sparovany->geo.X4))) || (F->scGPCheckBox_meridlo_casy->Checked && C->n>1) || (F->scGPCheckBox_meridlo_casy->Checked && (C->Element!=v.MAG_LASO->sparovany || (C->Element==v.MAG_LASO->sparovany && v.MAG_LASO->sparovany!=NULL && v.MAG_LASO->Element->geo.X1!=v.MAG_LASO->sparovany->geo.X4))))
				{
					...
					if((!prichyceno || (prichyceno && C->Element!=F->pom_element)) && v.vrat_druh_elementu(C->Element)==0)
					{
						...
						//latence
						pom_pohon=NULL;if(C->Element->dalsi!=NULL)pom_pohon=C->Element->dalsi->pohon;if(C->Element->dalsi==NULL)pom_pohon=F->d.v.ELEMENTY->dalsi->pohon;//načtení pohonu do pomocného ukazatele
						if(pom_pohon!=NULL)in.y+=m.latence_mezi_stopkami(pom_pohon->aRD);
						//PT robotů / operátorů
						if(C->Element->eID!=0){in.x+=C->Element->PTotoc+C->Element->data.PT1+C->Element->data.PT2;in.y+=C->Element->PTotoc+C->Element->data.PT1+C->Element->data.PT2;}
						//pro již změřený prvek srovnat
						out.x=in.x;out.y=in.x;
            //RT
					...
					if(prichyceno && C->dalsi==NULL && C->Element==F->pom_element && v.vrat_druh_elementu(F->pom_element)==0)
					{
						//WT
						out.y+=F->pom_element->WT;
						//latence
						pom_pohon=NULL;if(F->pom_element->dalsi!=NULL)pom_pohon=F->pom_element->dalsi->pohon;if(F->pom_element->dalsi==NULL)pom_pohon=F->d.v.ELEMENTY->dalsi->pohon;//načtení pohonu do pomocného ukazatele
						if(pom_pohon!=NULL)out.y+=m.latence_mezi_stopkami(pom_pohon->aRD);
						//PT robotů / operátorů
						if(F->pom_element->eID!=0){out.x+=F->pom_element->PTotoc+F->pom_element->data.PT1+F->pom_element->data.PT2;out.y+=F->pom_element->PTotoc+F->pom_element->data.PT1+F->pom_element->data.PT2;}
						//RT
      ...
      //pokud je první segment část oblouku, vykresluje se od konce, změna parametrů
			if(C->n==1 && C->Element->geo.typ!=0)
			{
				...
				OR=m.Rt15(C->Element->geo.orientace-C->Element->geo.rotacni_uhel-180);//obrácení orientace
				if(C->Element->geo.rotacni_uhel>0)uhel=-uhel;//změna směrování RA/úhlu
				typ=2;
			}
			//vytvoření popisků
			String t1="",t2="";
			if(prichyceno  && C->dalsi==NULL && (F->pom_element==v.MAG_LASO->predchozi->Element || (F->pom_element->eID==301 && F->pom_element->predchozi2==v.MAG_LASO->predchozi->Element)))
			{
				t1=String(m.round2double(delka*1000,2))+" [mm]";
				if(v.vrat_druh_elementu(F->pom_element)==0)
				{
					if(in.x!=in.y)t2="IN "+String(m.round2double(in.x,2))+" - "+String(m.round2double(in.y,2))+" [s]";else t2="IN "+String(m.round2double(in.x,2))+" [s]";
					if(out.x!=out.y)t2+="  OUT "+String(m.round2double(out.x,2))+" - "+String(m.round2double(out.y,2))+" [s]";else t2+="  OUT "+String(m.round2double(out.x,2))+" [s]";
				}
				else
				{
					if(in.x!=in.y)t2=String(m.round2double(in.x,2))+" - "+String(m.round2double(in.y,2))+" [s]";else t2=String(m.round2double(in.y,2))+" [s]";
				}
	...
	if(F->pom_element!=NULL && (!prichyceno || (prichyceno && F->pom_element!=v.MAG_LASO->predchozi->Element)) && (F->pom_element->dalsi==v.MAG_LASO->predchozi->Element || F->pom_element->predchozi==v.MAG_LASO->predchozi->Element || (F->pom_element->predchozi->n==0 && F->pom_element->predchozi->predchozi==v.MAG_LASO->predchozi->Element) || (F->pom_element->predchozi->eID==301 && F->pom_element->predchozi->predchozi2==v.MAG_LASO->predchozi->Element) || (F->pom_element->predchozi->eID==301 && F->pom_element->predchozi->predchozi==v.MAG_LASO->predchozi->Element)))
	{
		...
		if(F->pom_element!=NULL && F->pom_element->geo.typ!=0 && v.MAG_LASO->dalsi!=NULL)
		{
			...
			{
				if(in.x!=in.y)popisek="IN "+String(m.round2double(in.x,2))+" - "+String(m.round2double(in.y,2))+" [s]";else popisek="IN "+String(m.round2double(in.x,2))+" [s]";
				if(out.x!=out.y)popisek+="  OUT "+String(m.round2double(out.x,2))+" - "+String(m.round2double(out.y,2))+" [s]";else popisek+="  OUT "+String(m.round2double(out.x,2))+" [s]";
			}
			else
			{
	...
	else if(v.MAG_LASO->Element!=NULL && (F->pom_element==NULL || (F->pom_element!=NULL && F->pom_element!=v.MAG_LASO->predchozi->Element && F->pom_element->predchozi2!=v.MAG_LASO->predchozi->Element && F->pom_element->predchozi->predchozi2!=v.MAG_LASO->predchozi->Element && F->pom_element->predchozi->predchozi!=v.MAG_LASO->predchozi->Element)))
	{
		...
		if(v.MAG_LASO->dalsi==NULL && v.MAG_LASO->sparovany!=NULL && F->pom_element!=NULL && F->pom_element->pohon!=NULL && v.MAG_LASO->sparovany==F->pom_element)
		{
			...
			{
				//WT
				out.x=in.x;out.y=in.y+F->pom_element->WT;
      ...
      if(delka==0 && F->prichytavat_k_mrizce==1 && v.MAG_LASO!=NULL && v.MAG_LASO->sparovany!=NULL && v.vrat_druh_elementu(v.MAG_LASO->sparovany)==0 && v.MAG_LASO->Element->geo.X2==v.MAG_LASO->Element->geo.X3)
	  	{
				...
				//latence
				pom_pohon=NULL;if(v.MAG_LASO->sparovany->dalsi!=NULL)pom_pohon=v.MAG_LASO->sparovany->dalsi->pohon;if(v.MAG_LASO->sparovany->dalsi==NULL)pom_pohon=F->d.v.ELEMENTY->dalsi->pohon;//načtení pohonu do pomocného ukazatele
				if(pom_pohon!=NULL)in.y+=m.latence_mezi_stopkami(pom_pohon->aRD);
				//PT robotů / operátorů
				if(v.MAG_LASO->sparovany->eID!=0){in.x+=v.MAG_LASO->sparovany->PTotoc+v.MAG_LASO->sparovany->data.PT1+v.MAG_LASO->sparovany->data.PT2;in.y+=v.MAG_LASO->sparovany->PTotoc+v.MAG_LASO->sparovany->data.PT1+v.MAG_LASO->sparovany->data.PT2;}
				//odečítání záporného RT = přičítání RT
				if(v.MAG_LASO->sparovany->data.RT<0){in.x-=v.MAG_LASO->sparovany->data.RT;in.y-=v.MAG_LASO->sparovany->data.RT;}
			...
			//nastavení pro vykreslení části úhlu
			uhel=0;short typ=3;
			TColor barva=clMeridlo;
			if(F->pom_element->geo.typ!=0)//oblouk potřeba vykreslit opačně a na 2x
			{
				uhel=m.T2Aarc(F->pom_element->geo.radius,m.delka(F->pom_element->geo.X4,F->pom_element->geo.Y4,X,Y));//výpočet úhlu od konce geo. elementu do počátku měření
				if(F->pom_element->geo.rotacni_uhel>0)uhel=-uhel;//změna směrování RA/úhlu
				azimut=m.Rt15(F->pom_element->geo.orientace-F->pom_element->geo.rotacni_uhel-180);//obrácení orientace
				delka_Pud=F->pom_element->geo.radius;
				X=F->pom_element->geo.X4;Y=F->pom_element->geo.Y4;//změna počátku vykreslení
				vykresli_Gelement(canv,X,Y,azimut,uhel,delka_Pud,clMeridlo,2,"","",2);//vykreslení měřidla bez popisku
				typ=2;barva=(TColor)RGB(255,69,0);//clKolej, nastavení pro druhé vykreslení měřidla
				uhel=m.T2Aarc(F->pom_element->geo.radius,m.delka(F->pom_element->geo.X4,F->pom_element->geo.Y4,F->akt_souradnice_kurzoru.x,F->akt_souradnice_kurzoru.y));//výpočt úhlu od konce geo. elementu do souřadnic kurzoru (pro druhé vykreslení barvou kolejí)
				if(uhel==0)uhel=0.1;//zamezení vykreslení 1m přímky při úhlu 0°
        if(F->pom_element->geo.rotacni_uhel>0)uhel=-uhel;//změna směrování RA/úhlu
			}
			//vykreslení
			vykresli_Gelement(canv,X,Y,azimut,uhel,delka_Pud,barva,2,String(m.round2double(delka*1000,2))+" [mm]",popisek,typ,clMeridlo);
		}
...
void Cvykresli::vykresli_meridlo_proti_trendu(TCanvas *canv,bool prichyceno)
{
	...
	////lineární měření
	if(v.MAG_LASO->Element!=NULL)// && v.MAG_LASO->predchozi->n==0)
	{
		//načtení parametrů
      ...
      TPointD in;
			in.x=0;in.y=v.MAG_LASO->sparovany->WT;
			//latence
			Cvektory::TPohon *pom_pohon=NULL;if(v.MAG_LASO->sparovany->dalsi!=NULL)pom_pohon=v.MAG_LASO->sparovany->dalsi->pohon;if(v.MAG_LASO->sparovany->dalsi==NULL)pom_pohon=F->d.v.ELEMENTY->dalsi->pohon;//načtení pohonu do pomocného ukazatele
			if(pom_pohon!=NULL)in.y+=m.latence_mezi_stopkami(pom_pohon->aRD);
      pom_pohon=NULL;delete pom_pohon;
			//PT robotů / operátorů
			if(v.MAG_LASO->sparovany->eID!=0){in.x+=v.MAG_LASO->sparovany->PTotoc+v.MAG_LASO->sparovany->data.PT1+v.MAG_LASO->sparovany->data.PT2;in.y+=v.MAG_LASO->sparovany->PTotoc+v.MAG_LASO->sparovany->data.PT1+v.MAG_LASO->sparovany->data.PT2;}
...
//						}      Xd=(X1+X2)/2.0;Yd=(Y1+Y2)/2.0; if(rotace==90 || rotace==270)Xd=X2;  if(rotace==0 || rotace==180)Yd=Y2;
//						if(rotace==p_rotace)rotace=m.Rt90(p_rotace-180);
//            canv->Pen->Style=psDot;//nastevení čarkované čáry
//						canv->MoveTo(Xd,Yd);   spojnice1=spojnice2=true;
//						canv->LineTo(m.L2Px(v.ELEMENTY->predchozi->geo.X4),m.L2Py(v.ELEMENTY->predchozi->geo.Y4));
//				  	//vykreslení šipek
//				  	sipka(canv,(Xd+m.L2Px(v.ELEMENTY->predchozi->geo.X4))/2.0,(Yd+m.L2Py(v.ELEMENTY->predchozi->geo.Y4))/2.0,m.azimut(Xd,Yd,m.L2Px(v.ELEMENTY->predchozi->geo.X4),m.L2Py(v.ELEMENTY->predchozi->geo.Y4))*(-1),true,3,clBlack,clWhite,pmNotXor);
		    	}
...
void Cvykresli::vykresli_dopravnik(TCanvas *canv, Cvektory::TZakazka *zakazka)
{
	...
	while(E!=NULL)
	{
		...
		if(F->Akce==F->Takce::GEOMETRIE && F->posledni_editovany_element!=NULL && ((F->posledni_editovany_element->eID==300) || (F->posledni_editovany_element->dalsi!=NULL && F->posledni_editovany_element->dalsi->eID==301 && F->posledni_editovany_element->dalsi->dalsi2!=F->posledni_editovany_element->dalsi->predchozi2)))editace_vetve=true;
		TColor clKolej=(TColor) RGB(255,69,0); if(F->OBJEKT_akt!=NULL && F->OBJEKT_akt->n!=E->objekt_n && F->Akce!=F->Takce::POSUN_TEPLOMER && !editace_vetve)clKolej=m.clIntensive(clKolej,m.get_intensity()/1.8);//zesvětlování neaktivních pohonů
		TColor clRetez=clBlack; if(F->OBJEKT_akt!=NULL && F->OBJEKT_akt->n!=E->objekt_n && F->Akce!=F->Takce::POSUN_TEPLOMER && !editace_vetve)clRetez=m.clIntensive(clRetez,m.get_intensity());//zesvětlování neaktivních pohonů
		if(F->Akce==F->Takce::POSUN_TEPLOMER && !v.obsahuje_segment_cesty_element(E,v.ZAKAZKA_akt)){clRetez=m.clIntensive(clRetez,m.get_intensity() && !editace_vetve);clKolej=m.clIntensive(clKolej,m.get_intensity()/1.8);}//zesvětlení neaktivních věcí
...
void Cvykresli::vykresli_vetev_dopravniku(TCanvas *canv)
{
	...
	if(F->OBJEKT_akt!=NULL && F->Akce==F->Takce::NIC && F->pom_element!=NULL && FormX->vykresli_vetev>0 && (F->pom_element->eID==300 || F->pom_element->eID==301))
	{
		//deklarace
		double R;
...
{ //scena 0 - vše do dynamické, scena 1 - implicitně statické elementy do statické scény, scena 2 - implicitně statické elementy do dynamické scény, scena 3 - implicitně dynamické elementy do statické scény, scena 4 - implicitně dynamické elementy do dynamické scény, 5 - zobrazit jenom popisky
...
//			}
		...
		case 301://spojka
		{
			if(F->scButton_zamek_layoutu->ImageIndex==68 /*&& (scena==0 || scena==1)*/)//pokud je odemčený layout jsou viditelné vyhýbky
			{
				...
				if((E->eID==300 && E->dalsi2==E->predchozi2 && ((F->Akce!=F->Takce::GEOMETRIE && F->Akce!=F->Takce::GEOMETRIE_LIGHT) || ((F->Akce==F->Takce::GEOMETRIE || F->Akce==F->Takce::GEOMETRIE_LIGHT) && E->objekt_n==F->OBJEKT_akt->n))) || (E->eID==301 && F->Akce!=F->Takce::GEOMETRIE && F->Akce!=F->Takce::GEOMETRIE_LIGHT && (!m.DisD(E->geo.X4,E->predchozi2->geo.X4) || m.DisD(E->geo.Y4,E->predchozi2->geo.Y4))))
				{
					////vykreslní čtverce
					canv->Pen->Color=barva;
	...
	try
	{
		...
		if(typ!=3)//pokud se nemá zobrazovat jenom popisek
		{
			...
			switch(eID)
			{
				...
				case 2: case 8: case 12: case 16: if(scena==0 || scena>=3) vykresli_stopku(canv,pX,pY,"","",typ,m.Rt90(rotace+180),stav);break;//robot se stopkou
				case 3: case 9: case 13: case 17: if(typ==1 && scena<=1) 	vykresli_lakovaci_okno(canv,lX,lY,LO1,OTOC_delka,LO2,DkRB,rotace,LO_pozice); vykresli_otoc(canv,scena,pX,pY,"","",5,typ,rotace,stav);break;//s pasivní otočí
				case 4: case 10: case 14:case 18: vykresli_otoc(canv,scena,pX,pY,"","",6,typ,m.Rt90(rotace+180),stav);break;//s aktivní otočí (tj. s otočí a se stopkou)
			}
...
void Cvykresli::vykresli_cloveka(TCanvas *canv,short scena,long X,long Y,AnsiString name,AnsiString short_name,short eID,short typ,double rotace,short stav,double LO1,double OTOC_delka,double LO2,Cvektory::TElement *E)
{
	...
	switch(eID)
	{
		...
		case 102: case 106: if(scena==0 || scena>=3) vykresli_stopku(canv,pX,pY,"","",typ,m.Rt90(rotace90+rotace2),stav);break;//robot se stopkou
		case 103: case 107: if(typ==1 && scena<=2) 	vykresli_lakovaci_okno(canv,X,Y,LO1,OTOC_delka,LO2,DkRBpx,m.Rt90(rotace90+180+rotace2));vykresli_otoc(canv,scena,pX,pY,"","",5,typ,m.Rt90(rotace90+rotace2),stav);break;//s pasivní otočí
		case 104: case 108: vykresli_otoc(canv,scena, pX,pY,"","",6,typ,m.Rt90(rotace90+rotace2),stav);break;//s aktivní otočí (tj. s otočí a se stopkou)
	}
	...
	////dynamická scéna + pokud se nemá zobrazovat jenom popisek
	if(typ!=3 && (scena==0 || 3<=scena))
	{
		////nastavení pera
...
void Cvykresli::vykresli_otoc(TCanvas *canv,short scena,long X,long Y,AnsiString name,AnsiString short_name,short eID,short typ,double rotace,short stav,Cvektory::TElement *E)
{
	...
	//pokud je otoč aktivní tj. se stopkou a pokud se nemá zobrazovat jenom popisek
	if(eID==6 && typ!=3 && (scena==0 || scena>=3))//a zároveň pokud je vše v dynamické nebo se jedná o zobrazení dynamické stop buď ve statické sceně nebo dynamické 3-4
...
TPointD *Cvykresli::vykresli_Gelement(TCanvas *canv,double X,double Y,double orientace,double rotacni_uhel,double radius,TColor color,float width,String Text,String Text2,short typ,TColor colorZarazka,bool gdiplus)
{
	...
	if(Text!="" || Text2!="")
	{
		...
		if(colorZarazka!=NULL)canv->Font->Color=colorZarazka;//úprava MV 18.1.2021, pro potřeby měřidla
	}
	...
	if(F->OBJEKT_akt!=NULL && F->Timer_simulace->Enabled==false)//pokud není editovaný nějaký objekt nebo při timeru aplikace se tabulky nezobrazí
	{
		...
		if(F->OBJEKT_akt->element!=NULL)
		{
			...
			while(E!=NULL)
			{
				...
				//posun na další element
				if(F->predchozi_PM!=NULL && E==F->predchozi_PM)E=F->OBJEKT_akt->element;//pokud jsem začal na předchozím PM posunu se na první element v objektu
...
void Cvykresli::vykresli_kotu(TCanvas *canv,Cvektory::TElement *Element_do,bool AAsize)
{
	...
	{
		if(!F->posun_dalsich_elementu && F->pom_element!=NULL && ((F->JID==-11 || F->JID==-101) && F->pom_element->n==Element_do->n))highlight=1;//v případě, že není požadován posun dalších elementů, zvýrazní i kótu následujícího elementu, že se bude také měnit
		if((F->JID==-11 && F->pom_element!=NULL && F->pom_element->n==Element_do->n) || F->JID==13)highlight=1;//když se bude editovat hodnota kóty, nebo se bude kóta posouvat, kvůli následnému zaokrouhlování musí bohužel zůstat tady
	}
	//////probíhá editace kót
	if(F->Akce==F->GEOMETRIE || F->Akce==F->GEOMETRIE_LIGHT)
	{
...
//			if(F->index_kurzoru==-9 && X1==X2)if(F->editovany_text=="")Text="";else Text=F->editovany_text;//pro svislou kótu
	...
	{                                                               //v případě, že není požadován posun dalších elementů, zvýrazní i kótu následujícího elementu, že se bude také měnit
		if(aktElement!=NULL && ((F->JID==-11 && F->pom_element!=NULL && F->pom_element->n==aktElement->n) || (!F->posun_dalsich_elementu && aktElement!=NULL && F->JID==-11 && F->pom_element!=NULL && F->pom_element->n==aktElement->predchozi->n) || ((F->JID==-8 || F->JID==-9) && F->MOD==F->EDITACE) || F->JID==-5 && F->OBJEKT_akt!=NULL || F->JID==-2 && F->OBJEKT_akt==NULL))canv->Font->Style = TFontStyles()<< fsBold;//pouze když se mění hodnota kóty
...
//------------------------------------------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------------------------------------
//vykreslení časových os
void Cvykresli::vykresli_casove_osy(TCanvas *canv,short scena)
{
	short olY=F->scGPPanel_mainmenu->Height*3+oY;//offset lokální od horního okraj
	Cvektory::TSIM *sim=v.SIM->dalsi;
	while(sim!=NULL)
	{
		//vykreslení konkrétního časového procesu
		if(scena==0 || scena==1)//do statické scény
		{
			String N=sim->S->short_name.SubString(1,1)+sim->S->short_name.SubString(sim->S->short_name.Length(),1);
			switch(sim->A)//vyháknutý -1, čeká na palec 0, jede 1
			{
				case -1: vykresli_proces(canv,N,clRed,0,sim->T1*PX2SEC,sim->T2*PX2SEC,sim->V->n*KrokY+olY,true);break;//zastaven na stop stanici
				case 0:  vykresli_proces(canv,N,clWebOrange,0,sim->T1*PX2SEC,sim->T2*PX2SEC,sim->V->n*KrokY+olY,true);break;//čekání na palec, otevření stop stanice
				//case 1:  vykresli_proces(Canvas,"START",clGreen,0,sim->T1*d.PX2MIN,sim->T2*d.PX2MIN,sim->V->n*d.KrokY+oY,true);break;//uchycen na palec a zavření stop stanice
				case 2:  vykresli_proces(canv,""/*"přejezd"*/,m.clIntensive(clGreen,220),1,sim->T1*PX2SEC,sim->T2*PX2SEC,sim->V->n*KrokY+olY,true);break;//přejezd
			}
		}
		//zvýraznění (kolečko) na časových osách v momentu, kdy nastane děj
		if((scena==0 || scena==2) && m.round2double(sim->T1,1)==m.round2double(F->CAS,1))//do dynamické scény
		{
			double X=sim->T1*PX2SEC;double Y=sim->V->n*KrokY+olY;short s=8*3;
			set_pen(canv,clMeridlo,10,PS_ENDCAP_FLAT);
			//canv->Brush->Style=bsSolid;
			canv->Ellipse(X-s,Y-s,X+s,Y+s);
		}
	...
	if(scena==0 || scena==1)vykresli_Xosy(canv); //do statické - ale možná také mimo AA
}
//------------------------------------------------------------------------------------------------------------------------------------------------------
//vykreslí jeden dílčí časový proces časových os (obdelníček procesu objektu) pro jeden vozík
...
void Cvykresli::vykresli_Xosy(TCanvas *canv)
{
	...
	canv->Pen->Style=psSolid;//DashDot;//nastevení čarkované čáry
	canv->Pen->Color=TColor RGB(60,60,60);//TColor RGB(200,200,200);   //míchání světlě šedé
	canv->Brush->Style=bsClear;
	canv->Font->Color=clGray;
	canv->Font->Size=10*3;
	canv->Font->Name=F->aFont->Name;
	...
	short o=1;
	if(PosunT.x>10)o=-30;
	canv->TextOutW(o-PosunT.x*3,F->scGPPanel_mainmenu->Height*3,"voz|[s]"); //popisek osy x

	//svislice x sekundách
	int start=PX2SEC*20;//if(PosunT.x>0)start=0;
	for(int i=start;i<=v.SIM->predchozi->T2*PX2SEC;i+=PX2SEC*20)//po x sekundách
	{
		canv->MoveTo(i/*-PosunT.x*3*/,F->scGPPanel_mainmenu->Height*3+oY*3);
		canv->LineTo(i/*-PosunT.x*3*/,F->scGPPanel_mainmenu->Height+oY*3+(v.SIM->predchozi->V->n+1)*KrokY);//-1 pouze optická korekce
		canv->Brush->Style=bsSolid;
		canv->Brush->Color=clWhite;
		TextOut(canv,i,F->scGPPanel_mainmenu->Height*3,i/PX2SEC,CENTER,TOP);
	}
}
//------------------------------------------------------------------------------------------------------------------------------------------------------
...
//	//if(0<V && V<=v.VOZIKY->predchozi->n) //pokud se jedná o číslo vozík od min do max vozíků
//	{
//		vykresli_zamerovac_casovych_os(F->Canvas,X,Y);//nový přístup v zobrazování svislic, jen v momentu zobrazování labalu_zamerovac
//		//F->scGPLabel_zamerovac->Transparent=false;
//		//F->scGPLabel_zamerovac->Color=clWhite;
//		F->scGPLabel_zamerovac->Font->Color=(TColor) RGB(100,100,100);
//		F->scGPLabel_zamerovac->Left=X+5; F->scGPLabel_zamerovac->Top=Y+20; //+ odsazení
//		F->scGPLabel_zamerovac->Caption=" vozík: "+String(V)+" \n min: "+String((X+PosunT.x)/PX2MIN)+" ";
//		F->scGPLabel_zamerovac->Visible=true;//zobrazit
//	}
//	//else F->scGPLabel_zamerovac->Visible=false;//jinak zase skryje
}
//------------------------------------------------------------------------------------------------------------------------------------------------------
//vykreslí pohyblivou svislici na časové osy dle umístění kurzoru myši
void Cvykresli::vykresli_zamerovac_casovych_os(TCanvas *canv,int X,int Y)
{
