...
void Cvykresli::vykresli_legendu_casovych_os(TCanvas *canv)
{
	if(Form1->scGPGlyphButton_close_legenda_casove_osy->GlyphOptions->Kind==scgpbgkDownArrow)//pokud je legenda zobrazena + napozicování dle toho, zda jsou zobrazeny či skryty grafy
	{
		...
		TColor C=clRed;//odstaveno pro ZDMv.vrat_vozik(Voz+1)->zakazka->barva;
...
void Cvykresli::vykresli_vyrobky(TCanvas *canv,double X,double Y,double dJ,double sJ,double orientaceP,double rotaceJ,TColor clJig,float Width)
{
	...
	double M=-W/2/3*1.8;-W/2/3;//mezera mezi výrobky,poměrově vůči výrobku
	...
	TPointD O=m.rotace(0,0,m.o2r(orientaceP)+rotaceJ);m.rotace(-H/2.0,0,m.o2r(orientaceP)+rotaceJ);//odsazení od horizontálního středu, polovina výšky výrobku, zajistí vycentrování
