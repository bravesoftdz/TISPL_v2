...
private:
  ...
  unsigned short int Nacist_podklad(UnicodeString soubor);//realizuje nacteni podkladu
	...
	void log(AnsiString Text);//zapÌöe log do textovÈho souboru a p¯id· datum
	...
	bool el_vkabine(int X,int Y,int element_id,Cvektory::TElement *E=NULL);//vracÌ zda se element nach·zÌ na pohonu jakÈhokoliv sklonu, dvojÌ zp˘sob pouûitÌ: zad·nÌ parametru X,Y,eID pokud nem·m ukazatel na element nebo 0,0,0,ukazatel, vr·tÌ true nebo false
	bool nahled_ulozen;
...
public:		// User declarations
  ...
  void aktualizace_ComboPohon ();
	...
	double vrat_hranici(int mimo);//vracÌ max Ëi min hodnoty x a y sou¯adnic element˘ v kabinÏ
