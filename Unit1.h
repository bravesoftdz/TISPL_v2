...
private:
	...
	void TimerKill(UINT wTimerID);//ukonèí danı (dlew TimerID) timer
...
private:
  ...
  TRect oblast_prekryti_mGridu(Cvektory::TElement *E=NULL);//vrátí oblast pro kontrolu pøekrytí mGridù
	...
	bool pripnuti_dalsich_objektu(double citlivost=1.5);//pokud pøi uloení editovaného objektu je detekováno, e konec objketu nenavazuje na zaèátek následujísího objektu je poloen dotaz a po potvrzení dojde ke spojení
	void spojeni_prvni_posledni(double citlivost=1.5);//kontrola zda na sebe první a polední objekt navazují, pokud jsou blízko u sebe, ale nenavazují - naváe je
	bool napojeni_vedlejsi_vetve(Cvektory::TElement *e_posledni,bool kontrola_vzdalenosti=true);//provede kontrolu, zdá je monost geometrii spojit, dotáe se a spojí geometrii pokud uivatel souhlasí
  void vypni_geometrii();//vypne akci geometrie
	...
	void vytvor_seznam_pouzivanych_pohonu();//vytvoøí seznam, kam se zapíší pohony, které jsou pouívány jinde
	void smaz_seznam_pouzivanych_pohonu();//smae seznam pouívanıch pohonù
  ...
  unsigned long *pouzivane_pohony;//ukládá n pohonù, které jsou pouívané
...
public:		// User declarations
  ...
  int zpravy_backup_width;
	...
	String data_teplomeru;//slouí k uchování dat do buòìk, vyplnìní dat a na konci
  unsigned int radku_teplomeru;//uchovává v sobì poèet øádkù teplomìru
	...
	TRect vrat_max_zobrazitelnou_oblast();//vrátí max zobrazitelnou (na monitoru) oblast z metody vıše uvedené vrat_max_oblast
  ...
  double inDO (double outDO);
	...
	TPointD bod_vlozeni_elementu(double kontr_x=-1000,double kontr_y=-1000);//vrací bod vloení elementu, "pøilepuje" kurzor na geometrii pokud se jedná o pøímku, parametry kontr_x a y slouí ke kontrole bodu zda se nachází na pøímce (pøi posunu)
	...
	void posun_na_element(Cvektory::TElement *E);//pøesune se na dannı element
	void smaz_kurzor();
	...
	bool prohodit_sloupce_PM(Cvektory::TElement *E);//rozhodne zda mají bıt prohozeny sloupce PM, podle trendu geometrie
	...
	bool mail(String Host,String Username,String Password,String FromAddress,String FromName,String Subject,String Body,String To,String ccTo="",String bccTo="",String FileName="",String FileName2="");//odešle e-mail, doruèitel na všech tøech úrovní To,ccTo,bccTo mohou bıt mnohonásobnì zadaní, pouze odìlené èárkou, tìlo e-mailu lze zadat jako html, návratová hodnota vrací úspìšnost provedení
  ...
  void pridej_radek_tab_teplomeru(Cvektory::TElement *E,double cas,double WT,bool prejezd,bool celkem=false);
	...
	void reset_teplomeru(Cvektory::TObjekt *Objekt);//zborazí upozornìní, e došlo ke zmìnì geometrie a resetuje oblasti teplomerù
	...
	bool je_pohon_pouzivan(unsigned long n);//provede kontrolu, zda je pohon v seznamu pouívanıch, vrátí vısledek
	void rozmisti_mGridy();//rovnomìrnì rozmístí mGridny, poèátek je left top
  void vypocet_WT(Cvektory::TElement *E);//vypoèíta WT pro PM, vıhybku a spojku
  };
//---------------------------------------------------------------------------
