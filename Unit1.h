...
public:
	...
	enum Takce{NIC=0,PAN,PAN_MOVE,ZOOM_W,ZOOM_W_MENU,ADD,MOVE,VYH,MEASURE,KALIBRACE,ADJUSTACE,MOVE_ELEMENT,MOVE_TABLE,OFFSET_KOTY,MOVE_KOMORA,ROZMER_KOMORA,DRAW_HALA,MOVE_HALA,MOVE_BOD,MOVE_USECKA,MOVE_TEXT,GEOMETRIE};Takce Akce;
...
private:
  ...
  enum Tlanguage{EN,MN,CS};Tlanguage language;
	...
	void vlozit_predavaci_misto();//projde elementy a objekty, pokud je nÏkde nutnost vloûit p¯ed·vacÌ mÌsto vloûÌ ho tam
	short rotace_symbol(short trend,int X_bod,int Y_bod);//dle toho, zda je umisùovan˝ element nad osou Ëi pod osou pohonu je vr·cena rotace symbolu, X_bod,.. je bbod vkl·d·nÌ elementu (jedna sou¯adnice ho v·ûe na pohon)
	void vytvoreni_tab_knihovna();//vytovo¯enÌ tabulky knihovny objekt˘
	void popisky_knihovna_nahled(bool knihovna);//p¯epÌn· popisky mezi knihovnou a editacÌ
  ...
  short najdi_popisky_PM(double X,double Y,Cvektory::TObjekt *Objekt);//najde na jakÈm popisku u PM se nach·zÌ kurzor, vracÌ 0: nenalezeno, 1: pohon 1, 2: pohon 2
  ...
  void mGrid_mimo_obraz(Cvektory::TElement *E);//kontrola zde je mGrid mimo obraz, pokud ano vypnutÌ komponent
	...
	void zmena_editovaneho_objektu();//slouûÌ k p¯echodu z editace jednoho objektu do editace druhÈho objektu
	TPoint uprav_bod_vlozeni_elementu(TPoint bod_vlozeni,short rotace_symbolu,int eID=-1);//upravÌ bod kurzoru pro vloûenÌ elemntu na bod vykreslenÌ elementu (robot na konci ramena)
  ...
  void vlozeni_editace_geometrie();//vkl·d· novou geometrii nebo edituje jiû st·vajÌcÌ geometrii
	void ukonceni_geometrie();//ukonËenÌ akce geometrie a p¯ÌpadnÈ uzav¯enÌ kruhu
...
public:		// User declarations
	...
	Cvektory::TElement *pom_element,*pom_element_temp,*posledni_editovany_element,*element_temp;//element_temp je nulov·n p¯i kaûdÈm p¯ejetÌ kurzoru pouûÌv·n na vÏci kolem PM
  ...
  void NP_input(); // vol· zobrazenÌ PO - nahrazuje NP a NPin
	...
	void tab_knihovna_click(double X,double Y,long Col=-1,long Row=-1);//klik do knihovny objekt˘, spouötÏnÌ akce p¯id·v·nÌ objektu
  ...
  double inDO (double outDO);
	...
	TPoint bod_vlozeni_elementu(double kontr_x=-1000,double kontr_y=-1000);//vracÌ bod vloûenÌ elementu, "p¯ilepuje" kurzor na geometrii pokud se jedn· o p¯Ìmku, parametry kontr_x a y slouûÌ ke kontrole bodu zda se nach·zÌ na p¯Ìmce (p¯i posunu)
	bool bod_na_geometrii(double X, double Y,Cvektory::TElement *Element=NULL);//kontroluje zde se bod nach·zÌ na geometri, vracÌ pouze ano/ne, pokud je do metody posl·n ukazatel na element provÏ¯Ì zda se tento element nach·zÌ na geometrii
	double max_voziku(Cvektory::TElement *stopka);//vr·tÌ maxim·lnÌ moûn˝ poËet vozÌk˘ na stopce, podle geometrie p¯ed nÌ
