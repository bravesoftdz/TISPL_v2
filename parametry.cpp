...
void __fastcall TForm_parametry::scComboBox_rezimChange(TObject *Sender) {
		...
		if (scComboBox_rezim->ItemIndex != 0) // S&G  neøeší délku mezery
		{
				...
				if (Form_parametry->scComboBox_rotace->ItemIndex == 1)
						dV = Form1->d.v.PP.sirka_voziku; // pokud je požadován šíøka jigu
				if (P != NULL) { // je "zbytek po dìlení"
						if (P->roztec > 0) // pokud existuje rozteè
						{
								// scGPButton_OK->Enabled=false;
								double doporuc_mezera =
										Form1->m.mezera_mezi_voziky(dV, P->roztec, 0);
...
void TForm_parametry::setForm4Rezim(unsigned short rezim) {
		...
		if (Form1->STATUS == Form1->NAVRH)
				rezim += 10; // posunutí o 10 vytváøí režim+navrháø
		switch (rezim) {
		case 0: // STOP & GO
				{
						set(POHON, READONLY);
		...
		scGPButton_storno->Top = Form_parametry->Height -
				scGPButton_storno->Height - 10;
		scGPGlyphButton_paste->Top = Form_parametry->Height -
				scGPGlyphButton_paste->Height;
		scGPGlyphButton_copy->Top = scGPGlyphButton_paste->Top -
				scGPGlyphButton_copy->Height;
}
		...
		case TIME: // technologický èas v režimu návrháø
				{
						...
						case HIGHLIGHT
								...
								break;
						case ENABLED
										: /* scButton_zamek_CT->ImageIndex=38;CT_zamek=UNLOCKED; */
								break;
						case DISABLED:
								scGPNumericEdit_CT->Enabled = false;
								/* scButton_zamek_CT->ImageIndex=37;CT_zamek=LOCKED; */ break;
						case READONLY
										: /* ROSTA-styl//scGPNumericEdit_CT->Options->ShapeStyle=scgpessNone; */
								scButton_zamek_CT->Visible = false;
								scGPNumericEdit_CT->Enabled = false;
								break;
						case HIDE:
		...
		case RYCHLOST: // RD v režimu návrháø
				{
						...
						switch (S) {
						case HIGHLIGHT
										: /* ROSTA-styl//scGPNumericEdit_RD->Options->FrameNormalColor=hl_color;scGPNumericEdit_RD->Options->FrameWidth=hlFrameWidth; */
						case ENABLED
										: /* scButton_zamek_RD->ImageIndex=38;RD_zamek=UNLOCKED; */
								break;
						case DISABLED
										: /* scGPNumericEdit_RD->Enabled=false;scButton_zamek_RD->ImageIndex=37;RD_zamek=UNLOCKED; */
								break;
						case READONLY
										: /* ROSTA-styl//scGPNumericEdit_RD->Options->ShapeStyle=scgpessNone; */
...
// ---------------------------------------------------------------------------
void __fastcall TForm_parametry::scGPNumericEdit_delka_dopravnikuChange
		(TObject *Sender) {
		if (input_state == NOTHING && input_clicked_edit == DD_klik &&
				scComboBox_rezim->ItemIndex != 0) { // pro Kontinual a PP režim
		...
		if (scComboBox_rezim->ItemIndex != 0) // mimo S&G
		{
				...
				if (P != NULL) {
						...
						if (Form_parametry->RDunitT == Form_parametry->MIN)
								jednotky_cas_pohon = 60.0;
						else
								jednotky_cas_pohon = 1.0;
						if (scGPNumericEdit_RD->Value <
								P->rychlost_od * jednotky_cas_pohon ||
								P->rychlost_do * jednotky_cas_pohon <
...
void TForm_parametry::input_CT() {
		...
		{ // pokud je v režimu SG nevolá se výpoèetní model
						...
						// nutné ošetøení pro období zadávání/psaní
				{
						...
						////////naètení dat zpìt do formuláøe po výpoètu////////////
						LoadDataToFormFromMath(); // u SG toto vùbec nevolám
				}
		...
		{ // pokud je v režimu SG nevolá se výpoèetní model
				...
				if (CT > Form1->ms.MyToDouble(Form1->d.v.PP.TT)
						...
						if (fmod(CT, Form1->d.v.PP.TT) == 0) {
								...
								vypis(" Rozložit na " + AnsiString(kapacitaSG) + "x " +
						...
						// podmínky pouze pro režim SG!!
				{
						...
						else // na lince je více objektù, pokud mají nižší CT dovolím je uložit
						{
								scGPButton_OK->Caption = "Uložit";
								scGPButton_OK->Enabled = true;
								// ostatní objekty v poøadí na lince mohu uložit s nižším CT než je TT linky
								vypis("");
						}
				}
		}
...
void TForm_parametry::input_M() {
		input_state = mezera;
		LoadDataFromFormAndSave();
		if (scGPNumericEdit_mezera->Value >=
				0 && scComboBox_rezim->ItemIndex == 1)
				// nutné ošetøení pro období zadávání/psaní
		{
		...
		{ // pouze pøi režimu PP
				...
				if (mrOk == Form1->MB("Pøejete si zmìnit délku kabiny nebo kapacitu?",
						MB_OKCANCEL, true, 366, false))
...
void __fastcall TForm_parametry::scComboBox_pohonChange(TObject *Sender) {
		if (scComboBox_rezim->ItemIndex == 1)
				// S&G  a Postprocesní nejsou kontrolovány na rozsah
		{
...
void __fastcall TForm_parametry::scGPNumericEdit_mezeraChange(TObject *Sender) {
		...
		if (scComboBox_rezim->ItemIndex == 1 &&
				scGPNumericEdit_mezera->Value > 0) // pouze pro kontinuál režim
		{
				Cvektory::TPohon *P =
						Form1->d.v.vrat_pohon(scComboBox_pohon->ItemIndex);
				// mezera_mezi_voziky
				...
				if (scComboBox_rotace->ItemIndex == 1)
						dV = Form1->d.v.PP.sirka_voziku; // pokud je požadován šíøka jigu
				double doporuc_mezera = Form1->m.mezera_mezi_voziky(dV, P->roztec,
						scGPNumericEdit_mezera->Value);
				if (P != NULL) { // je "zbytek po dìlení"
						if (P->roztec >
								0 /* && !Form1->m.cele_cislo(scGPNumericEdit_mezera->Value/P->roztec) */
								) // nesplòuje rozmezí
						{
...
void TForm_parametry::LoadDataFromFormAndSave() {
		...
		if (CTunit == MIN)
				CT = CT * 60.0;
		// pokud bylo zadání v minutách pøevede na sekundy - jinak je CT v Si a mohu ho hned uložit k výpoètu
		if (RDunitT == MIN)
				RD /= 60.0;
		if (RDunitD == MM)
				RD *= 1000.0;
...
void TForm_parametry::LoadDataToFormFromMath() {
		...
		if (input_state != CT) {
				...
				if (CTunit == MIN)
						scGPNumericEdit_CT->Value /= 60.0;
		}
		if (input_state != RD) {
				scGPNumericEdit_RD->Value = pm.RD;
				if (RDunitT == MIN)
						scGPNumericEdit_RD->Value *= 60.0;
				else if (RDunitD == MM)
						scGPNumericEdit_RD->Value *= 1000.0;
		}
		if (input_state != DD) {
				scGPNumericEdit_delka_dopravniku->Value = pm.DD;
				if (DDunit == MM)
						scGPNumericEdit_delka_dopravniku->Value *= 1000.0;
		}
		if (input_state != C) {
				...
				if (!ikonka) {
						...
						if (E == mezera_klik) {
								if (scComboBox_pohon->ItemIndex != 0) {
										...
										scButton_zamek_CT->ImageIndex = 37;
										CT_zamek = LOCKED;
										scGPNumericEdit_RD->ReadOnly = true;
										scButton_zamek_RD->Visible = false;
										RD_zamek = LOCKED;
								}
						}
