...
//	void umisti_palec(TCanvas *canv,Cvektory::TPalec *ukaz);//zajišuje aktuální umístìní vozíku na lince vùèi animaci
	...
	void vykresli_kotu(TCanvas *canv,double X1,double Y1,double X2,double Y2,double Offset=0,bool highlight=false,float width=0.2,TColor color=clGray);//v metrickıch jednotkách kromì width, zde v px + automaticky dopoèítává délku a dosazuje aktuálnì nastavené jednotky
	void vykresli_kotu(TCanvas *canv,long X1,long Y1,long X2,long Y2,AnsiString Text,int Offset=0,bool highlight=false,float width=0.2,TColor color=clGray);//v px + dosazuje aktuálnì nastavené jednotky
