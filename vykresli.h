...
class Cvykresli
{
  ...
  bool prichyceno_na_vozik;
	...
	void vykresli_meridlo(TCanvas *canv,int X,int Y,bool kalibracni_sipka=false);//v pøípadì mìøení vzdálenosti vykreslí spojnici a popø. vypisuje hodnotu vzdálenosti, pouívá se asi i na kalibraci podkladu
  ...
  bool vykresli_cit_oblasti_lasa(TCanvas *canv);//vykreslí citelné oblasti elementù na které je moné se pøichytit, vrací true, pokud byla oblast vykreslena
  ...
  void vykresli_vetev_dopravniku(TCanvas *canv);//vykreslení highlightu akt. editované vìtve
	...
	void vykresli_elementy(TCanvas *canv,short scena);//cyklem prochází všechny elementy a ty následnì "selectí" dle: scena 0 - vše do dynamické, scena 1 - implicitnì statické elementy do statické scény, scena 2 - implicitnì statické elementy do dynamické scény, scena 3 - implicitnì dynamické elementy do statické scény, scena 4 - implicitnì dynamické elementy do dynamické scény, 5 - zobrazit jenom popisky
	void vykresli_element(TCanvas *canv,short scena,long X,long Y,AnsiString name,AnsiString short_name,unsigned int eID=0,short typ=0,double rotace=0,short stav=1,double LO1=1.5,double OTOC_delka=0,double LO2=0,double LO_pozice=0,Cvektory::TElement *E=NULL);//celková vykreslovací metoda, vykreslí buï stopku, robota nebo otoè, scena 0 - vše do dynamické, scena 1 - implicitnì statické elementy do statické scény, scena 2 - implicitnì statické elementy do dynamické scény, scena 3 - implicitnì dynamické elementy do statické scény, scena 4 - implicitnì dynamické elementy do dynamické scény, 5 - zobrazit jenom popisky
  ...
  void vykresli_editovane_polozky(TCanvas *canv);//vykresli nad statickou scénu aktuálnì editované poloky, pouívá se pøi editaci textu, ale je zde bez AA
	...
	void smart_kurzor(TCanvas *canv,Cvektory::TElement *E=NULL,short typElementu=0);//volání smart kurzoru slouícího pro vıbìr geometrického elementu, to dle pøedchozích parametrù resp. typù geometrickıch elementu a zadanıch vstupního paramerù metody, tj. dle posledního existujícího elementu
	void smart_kurzor(TCanvas *canv,double preXk,double preYk,double preOR=0,double preRA=0,double prepreRA=0,short typElementu=0);//volání smart kurzoru slouícího pro vıbìr geometrického elementu, to dle pøedchozích parametrù resp. typù geometrickıch elementu a zadanıch vstupních paramerù metody, preXk,preYk - koncové souøadnice, preOR - orientace, preRA - rotaèní úhel pøedchozího geometrického elementu tj. posledního ji vytvoøeného, prepreRArotaèní úhel pøed-pøedchozího geometrického elementu - neexistují-li tyto pre resp. prepre element(y), zadat nulu
	void vykresli_Gelement_kurzor(TCanvas *canv,double X,double Y,double orientace,double rotacni_uhel,double radius,double delka_linie,double predchozi_rotacni_uhel,double predpredchozi_rotacni_uhel,short typElementu=0);//obloukovı èi liniovı (dle situace) kurzor g-elementu, X,Y jsou fyzické souøadnice vıchozího vykreslování, parametry: orientace oblouku - dle svìtovıch stran (umí i jiné ne 90° násobky), rotaèní úhel, pod kterım je oblouk rotován, mùe bıt zápornı (znaménko urèuje smìr rotace, + proti smìru hodinovıch ruèièek, - po smìru), max. hodnota +90 a min. hodnota -90 (je-li nastaven na 0° jedná se o linii), radius - je radius oblouku v metrech nebo pokud je rotaèní úhel nastaven na 0° tedy se jedná o linii, je radius délkou linie)
	TPointD *vykresli_potencial_Gelement(TCanvas *canv,double X,double Y,double orientace,double rotacni_uhel,double radius,TColor color,bool popisek);//zajistí jednorázové vykreslení potenciálního obloukového èi liniového (dle situace) g-elementu, X,Y jsou logické souøadnice vıchozího vykreslování, parametry: orientace oblouku - dle svìtovıch stran (umí i jiné ne 90° násobky), rotaèní úhel - pod kterım je oblouk rotován, mùe bıt zápornı (znaménko urèuje smìr rotace, + proti smìru hodinovıch ruèièek, - po smìru), max. hodnota +90 a min. hodnota -90 (je-li nastaven na 0° jedná se o linii), radius - je radius oblouku v metrech nebo pokud je rotaèní úhel nastaven na 0° tedy se jedná o linii, je radius délkou linie)
	TPointD *vykresli_Gelement(TCanvas *canv,double X,double Y,double orientace,double rotacni_uhel,double radius,TColor color,float width,String Text="",String Text2="",short typ=-1,TColor colorZarazka=NULL,bool gdiplus=false);//zajistí jednorázové vykreslení libovolného obloukového èi liniového (dle situace) g-elementu, X,Y jsou logické souøadnice vıchozího vykreslování, parametry: orientace oblouku - dle svìtovıch stran (umí i jiné ne 90° násobky), rotaèní úhel - pod kterım je oblouk rotován, mùe bıt zápornı (znaménko urèuje smìr rotace, + proti smìru hodinovıch ruèièek, - po smìru), max. hodnota +90 a min. hodnota -90 (je-li nastaven na 0° jedná se o linii), radius - je radius oblouku v metrech nebo pokud je rotaèní úhel nastaven na 0° tedy se jedná o linii, je radius délkou linie), typ=-1 jen støedová èára, typ=0 jen koleje, typ 1=koleje+zaráka na zaèátku, typ 2=koleje+zaráka na konci, typ 3=na zaèátku i na konci, barva colorZarazka nastaví pro pøípadnì zobrazovanou zaráku speciální barvu, pokud není parametr pouit (je NULL), tak dostane pøípadná zaráka stejnou barvu jako barva parametru color
  ...
  void vykresli_ikonu_stoupani_klesani(TCanvas *canv,int X,int Y,short typ,String popisek="",short stav=0);
	...
	void set_pen2(TCanvas *canv, TColor color, int width, int ENDCAP=PS_ENDCAP_SQUARE,int JOIN=PS_JOIN_MITER,bool INSIDEFRAME=true,DWORD *UserLineArray=NULL,unsigned short LenghtArray=0,int TextureWidth=0);//vrátí HANDLE na nastavení pera,//popø.PS_ENDCAP_FLAT PS_ENDCAP_ROUND, PS_ENDCAP_SQUARE a PS_JOIN_ROUND, PS_JOIN_MITER (lze nastavit pøes SetMiterlimit) PS_JOIN_BEVEL, viz Matoušek III str. 179 èi http://www.zive.cz/clanky/geometricka-pera/sc-3-a-103079, dodáním pøedposledního parametru UserLineArray napø. DWORD pole[]={m.round(3/3.0*F->Zoom),m.round(0.5/3.0*F->Zoom),m.round(0.2/3.0*F->Zoom),m.round(0.5/3.0*F->Zoom)} apod.  lze nadefinovat vlastní typ linie, poslední parametr je poèet prvkù onoho pole
	TColor set_color(TCanvas *canv, Cvektory::TObjekt *O);
	void drawRectText(TCanvas *canv,TRect Rect, UnicodeString Text);
	void TextFraming(TCanvas *canv,int X,int Y,UnicodeString Text,TFont *Font=NULL,TColor clFraming=clWhite,unsigned short FramingSize=4);//zajistí vykreslení daného textu dle nastaveného Fontu (pokud je NULL, pøevezme se akutální font canvasu) vèetnì framingu, kterı je baravnì a velikostnì nastavitelnı
	HRGN TextOut(TCanvas *canv,int X,int Y,UnicodeString Text,Talign align=CENTER,Tvalign valign=MIDDLE,float FaktorLineSpacing=1,TColor clFraming=clWhite,unsigned short FramingSize=4,TFont *Font=NULL);//zajistí vykreslení daného textu dle nastaveného Fontu (pokud je NULL, pøevezme se akutální font canvasu), mono vertikálnì i horizontálnì pozicovat, umí zalamovat øádky v pøípadì vıskytu \n, velikost odøádkování dle velikosti písma 1=100%, v pøípadì záporné hodnoty øazení textu od prvního øádku dalším smìrem nahoru, umí také framing, kterı je baravnì a velikostnì nastavitelnı, neumí zatím rotaci textu!!!
	Graphics::TBitmap *srafura(TColor color);//vytvoøí šrafovanı vzor
	Graphics::TBitmap *rastrovani(TColor color,short width, short distance);//vytvoøí rastrovanı vzor
	...
	void sipka(TCanvas *canv,int X, int Y,float azimut,short typ=0, float size=3, COLORREF color=clBlack,COLORREF color_brush=clGray,TPenMode PenMode=pmCopy,TPenStyle PenStyle=psSolid,bool teziste_stred=true);//zajistí vykreslení šipky, typ 0 - barevná vıplò, 1 - transparentní vıplò (bez vıplnì), 2 - transparentní vıplò (bez vıplnì) a bez znázornìní hrany pøepony trojúhleníku tvoøící šipku, pozor velikost šipky ji nenásobit zoomem
	...
	void vykresli_casove_osy(TCanvas *canv,short scena);//vykreslení èasovıch os
	void vykresli_proces(TCanvas *canv, AnsiString shortname, TColor color, short typ, long X1, long X2, long Y,bool legenda);//vykreslí jeden dílèí èasovı proces èasovıch os (obdelníèek procesu objektu) pro jeden vozík
	...
	void vykresli_svislici_na_casove_osy(TCanvas *canv,int X=-1,bool AA=false);//v aktuálním èase simulace vykreslí svislici aktuální události na èasovıch osách
	...
	//short NOLIEX;//NO - 0, LINEAR - 1, EXPO - 2  znázornìní vytíení oobjektu
	...
	//TRect aktOblast;//aktuální citelná oblast popisku elementu urèená k uloení ji øešeno pøímo v elementu
	...
	unsigned long zprava_highlight;//pomocná promìnná slouící na ztuènìní dané zpravy
	long SCENA;//ZprVozEledElesDopObjHal
	bool GDIplus;//povoluje èi zakazuje GDI+ vykreslování tam, kde je implementováno
