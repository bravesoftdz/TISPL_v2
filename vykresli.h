...
//	void umisti_palec(TCanvas *canv,Cvektory::TPalec *ukaz);//zajišuje aktuální umístìní vozíku na lince vùèi animaci
  ...
  void vykresli_zarazku(TCanvas *canv,Cvektory::TElement *E);
	...
	void smart_kurzor(TCanvas *canv,Cvektory::TElement *E=NULL,Cvektory::TElement *Ep=NULL);//volání smart kurzoru slouícího pro vıbìr geometrického elementu, to dle pøedchozích parametrù resp. typù geometrickıch elementu a zadanıch vstupních paramerù metody, tj. dle posledního existujícího elementu a jeho pøechozího elementu (vyadováno zadávání druhého parametru min. do zmìny DM, u vıhybek se ukáe..., pøipadnì druhı parametr zrušit), lze zadavat jako oba parametry NULL, pokud nejsou pøedchozí elementy k dispozici
