...
//	bool KOLIZE(Cvektory::TVozik *V1,Cvektory::TVozik *V2);//vrací logickou hodnotu zda došlo èi nedošlo ke kolizi s jinım vozíkem
	...
	void vykresli_pozice(TCanvas *canv,double X,double Y,double orientace,unsigned int pocet_pozic,unsigned int pocet_voziku);//vykresli pozice na stop elementech
	...
	void vykresli_vozik(TCanvas *canv,int ID, double X,double Y,double dP,double dJ,double sJ,double orientaceP=0,double rotaceJ=0,TColor clChassis=(TColor) RGB(50,50,50), TColor clJig=clPurple);//vykreslení jednoho komplexního vozíku (podvozek vèetnì jigu)
...
//	void umisti_palec(TCanvas *canv,Cvektory::TPalec *ukaz);//zajišuje aktuální umístìní vozíku na lince vùèi animaci
	...
	void vykresli_zarazku(TCanvas *canv,Cvektory::TElement *E);
	void vykresli_predavaci_misto(TCanvas *canv,long X,long Y,AnsiString name,short typ=0,double rotace=0,short stav=1);
