...
class Cvykresli
{
	...
	void vykresli_koleje(TCanvas *canv,double X,double Y,short typ,double orientace,double rotacni_uhel,double radius,double delka,TColor clKolej=(TColor)RGB(255,69,0),short TypZarazky=0);//vykreslení jednoho geometrického segmentu dvou párù kolejí, TypZarazky=0 bez,1=na zaèátku,2=na konci
	...
	TPointD *vykresli_Gelement(TCanvas *canv,double X,double Y,double orientace,double rotacni_uhel,double radius,TColor color,float width,String Text="",String Text2="",short typ=-1);//zajistí jednorázové vykreslení libovolného obloukového èi liniového (dle situace) g-elementu, X,Y jsou logické souøadnice vıchozího vykreslování, parametry: orientace oblouku - dle svìtovıch stran (umí i jiné ne 90° násobky), rotaèní úhel - pod kterım je oblouk rotován, mùe bıt zápornı (znaménko urèuje smìr rotace, + proti smìru hodinovıch ruèièek, - po smìru), max. hodnota +90 a min. hodnota -90 (je-li nastaven na 0° jedná se o linii), radius - je radius oblouku v metrech nebo pokud je rotaèní úhel nastaven na 0° tedy se jedná o linii, je radius délkou linie), typ=-1 jen støedová èára, typ=0 jen koleje, typ 1=koleje+zaráka na zaèátku, typ 2=koleje+zaráka na konci
