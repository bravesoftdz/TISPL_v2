...
//	void umisti_palec(TCanvas *canv,Cvektory::TPalec *ukaz);//zajišuje aktuální umístìní vozíku na lince vùèi animaci
	...
	void TextFraming(TCanvas *canv,int X,int Y,UnicodeString Text,TFont *Font=NULL,TColor clFraming=clWhite,unsigned short FramingSize=2);//zajistí vykreslení daného textu dle nastaveného Fontu (pokud je NULL, pøevezme se akutální font canvasu) vèetnì framingu, kterı je baravnì a velikostnì nastavitelnı
