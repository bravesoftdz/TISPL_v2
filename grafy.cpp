...
void Cgrafy::graf2() {
    ...
    Form1->Chart2->Height = Form1->ClientHeight / 3 * 1;
		...
		Form1->Series2->Clear();
		Form1->Series3->Clear();
		// Form1->Series4->Clear(); // souèet technolog.èasù - celı zakázka vıroby
		...
		// if (Form1->d.PROZATIM) //nefunguje pøi prvním prùchodu a pøi druhém...
		// {
		...
		while (ukaz != NULL) {
				...
				if (Form1->d.v.vrat_AVG_TT_zakazky(ukaz) != ukaz->TT) {
						...
						// Form1->scHTMLLabel_log_vypis->Caption="Chyba u zakázky è."+zakazka_n+" - Uskuteènitelnı TT: <b>"+tt_uskutec+"</b>. Poadovanı TT: <b>"+tt_pozad+"</b>";
				}
...
void Cgrafy::graf3() {
		...
		// Form1->Chart3->Title->Caption = "Vytíenost zakázky";
		//
...
void Cgrafy::zpravy() {
	...
	//	ShowMessage(zakazka->TT);
		...
		zpravy += "Zakázka è." + id + "- " + name + ", poadovanı takt " + tt + " sekund <b>" + stav + "</b>";
		zprava_dlouha_probox+="Zakázka è." + id + "- " + name + ", poadovanı takt " + tt + " sekund <b>" + stav + "</b>";
		...
		if (avg_tt_zakazky > 0) {
				zpravy +=", vypoèítanı prùmìrnı takt " + avg_tt_zakazky + " s</br>";
				zprava_dlouha_probox+=", vypoèítanı prùmìrnı takt " + avg_tt_zakazky + " s</br>";
		}
	...
	if(takt_splnen){ sirka_boxu=450;} else sirka_boxu=715;
		...
		zpravy += "<br><b>Zobrazení rozdílnıch kapacit:</b></br>";
		Cvektory::TObjekt *ukaz = Form1->d.v.OBJEKTY->dalsi;
