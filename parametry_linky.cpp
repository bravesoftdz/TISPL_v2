...
void __fastcall TForm_parametry_linky::FormShow(TObject *Sender)
{
   ...
   // scHTMLLabel1->Caption="Ahojky - <bgcolor =clWhite>[mm]</bgcolor>";
     ...
     scGPSwitch->Enabled=false;
		...
		//nastavení caption
		if(Taktunit==1)scHTMLLabel_takt->Caption="[m]";
  ...
  PL_mGrid->Top=scGPButton_pohon->Top+scGPButton_pohon->Height ;//vhodné jako druhé (popø. by bylo nutné pøekreslovat)
	PL_mGrid->AntiAliasing_text=true;
	PL_mGrid->Border.Width=1;
  PL_mGrid->DefaultRowHeight=28; //vìtší výška øádku, kvùli velikosti comba - aby se vešlo celé
...
//		}
    ...
    //nastaveni rozmeru formu - dle poctu pohonu
     setFormHeight();
		...
		zrusena_prirazeni_PID_size=PL_mGrid->RowCount-1;//velikost staèí jako poèet øádkù/pohonu po naètení, více jich být pøiøazeno do nového naètení formu být nemùže
  ...
  PL_mGrid->Note.margin_top=1;
	 ...
	 Form_parametry_linky->Width=odsazeni+PL_mGrid->Width+odsazeni+1;//musí se ruènì pøidat jeden pixel!
	 //TOP
	 ...
	 scHTMLLabel_podlahovy->Top=scGPButton_vozik->Top+scGPButton_vozik->Height/2-scHTMLLabel_podlahovy->Height/2;
	 ...
	 scHTMLLabel_takt->Top=scPanel_takt->Height/2-scHTMLLabel_takt->Height/2+1;
	 //Left uvnitø
	 ...
	 //obrázky
	 ImageEx_jig_podlahovy->Left=scPanel_vozik->Width/2-ImageEx_jig_podlahovy->Width/2;
	 ImageEx_jig_podvesny->Left=scPanel_vozik->Width/2-ImageEx_jig_podvesny->Width/2;
	 //design tlaèítek
	 ...
	 if (ukaz!=NULL)
	 {
        ...
        PL_mGrid->Refresh();  //nutné
            ...
            PL_mGrid->Cells[5][i].Type=PL_mGrid->COMBO;
						PL_mGrid->Refresh();   //kvùli práci s combem je nutný refresh po nastavení na typ COMBO
         ...
         if(Form1->d.v.pohon_je_pouzivan(ukaz->n))
          ...
          PL_mGrid->Cells[5][i].Type=PL_mGrid->readEDIT;
					PL_mGrid->Cells[5][i].Type=PL_mGrid->COMBO;
					PL_mGrid->getCombo(5,i)->Enabled=false; //nechám typ combo, pouze ho zakážu
          ...
          PL_mGrid->Cells[5][i].InputNumbersOnly=2;
           ...
           PL_mGrid->Refresh(); // kvùli prací s následných Checkboxem je nutný refresh
...
//        }
      ...
      bool nelze_ulozit=false;
			...
			for (unsigned int i = 2; i < PL_mGrid->RowCount; i++)
			{
				...
				if (PL_mGrid->Cells[1][i].Text=="") nazev="nový pohon";
...
void __fastcall TForm_parametry_linky::Button_ADD_Click(TObject *Sender)
{
	...
	if(katalog_id!=0)
	{
  	...
  	int i = PL_mGrid->RowCount -1 ;//poøadí øádku o jednièku nižší než poèet øádkù

  	PL_mGrid->Cells[0][i].Text = getMaxPID()+1;//PL_mGrid->RowCount - 2;
		PL_mGrid->Cells[1][i].Text = "nový pohon ";//rStringGridEd_tab_dopravniky->Cells[1][i - 1];
...
//---------------------------------------------------------------------------
void __fastcall TForm_parametry_linky::scHTMLLabel_sirka_jigClick(TObject *Sender)
{
  F->log(__func__); //logování
	scHTMLLabel_delka_jigClick(Sender);
}
...
//---------------------------------------------------------------------------
void __fastcall TForm_parametry_linky::scHTMLLabel_delka_jigClick(TObject *Sender)
{
  F->log(__func__); //logování
...
void TForm_parametry_linky::vozik_zmena_jednotek()
{
  F->log(__func__); //logování
	...
	{
		scHTMLLabel_delka_jig->Left=scGPNumericEdit_delka_jig->Left+scGPNumericEdit_delka_jig->Width+2;
		scHTMLLabel_delka_podvozek->Left=scGPNumericEdit_delka_podvozek->Left+scGPNumericEdit_delka_podvozek->Width+2;
	}
	else
	{
		scHTMLLabel_delka_podvozek->Left=scGPNumericEdit_delka_jig->Left+scGPNumericEdit_delka_jig->Width+2;
		scHTMLLabel_delka_jig->Left=scGPNumericEdit_delka_podvozek->Left+scGPNumericEdit_delka_podvozek->Width+2;
	}
	scHTMLLabel_sirka_jig->Left=scGPNumericEdit_sirka_jig->Left+scGPNumericEdit_sirka_jig->Width+2;
	scHTMLLabel_vyska_jig->Left=scGPNumericEdit_vyska_jig->Left+scGPNumericEdit_vyska_jig->Width+2;
}
...
bool TForm_parametry_linky::existuji_nepouzivane_pohony()
{
   F->log(__func__); //logování
	 bool RET=false;
	 for(unsigned int i=2;i<PL_mGrid->RowCount;i++)//prochází všechny pohany a pokud je pohon nepoužíván, smažeho
	 {
...
void TForm_parametry_linky::nastav_edity ()
{
  F->log(__func__); //logování
	...
	if(ImageEx_jig_podlahovy->Visible&&scGPNumericEdit_delka_jig->Top>scGPNumericEdit_delka_podvozek->Top||ImageEx_jig_podvesny->Visible&&scGPNumericEdit_delka_jig->Top<scGPNumericEdit_delka_podvozek->Top)//podlahový vozík a zároveò špatné edity
	{
...
void TForm_parametry_linky::vypis(UnicodeString text,bool red,bool link)
{
 ...
 //if(text=="m].</b>")text="";//provizorní WA, pøi zmìnì Rz a byla-li v poøádku to vrací toto  - již není tøeba, ale zatím nechávám
		...
		{
				scHint1->ToString()=text;//natežení do hintu zajišuje zobrazení celého textu, nepoužívá se klasický hint
          ...
          TShiftState Shift, int X, int Y)
{
 F->log(__func__); //logování
   ...
   if(Button==mbLeft && PL_mGrid->CheckPTinTable(X,Y))      //pøepnutí jednotek
   {
      ...
      if(RET.x==4)
      {
       if(aRDunit==MIN) {	PL_mGrid->Cells[2][0].Text="Rozmezí a rychlost pohonu <a>[m/s]</a>";   aRDunit=S;      }
       else             { PL_mGrid->Cells[2][0].Text="Rozmezí a rychlost pohonu <a>[m/min]</a>"; aRDunit=MIN;    }
