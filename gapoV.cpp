...
void __fastcall TF_gapoV::FormActivate(TObject *Sender)
{
	...
	unsigned long ColCount=21;//pevný poèet slopcù
	...
	mGrid->Cells[12][0].Text="CT - Technologický èas [s]"; //mGrid->MergeCells(6,0,7,0);//slouèení zatím nefunguje dobøe
	mGrid->Cells[13][0].Text="RD - Rychlost pohonu [m/s]";
	mGrid->Cells[14][0].Text="DD - Délka objekt [m]";
	mGrid->Cells[15][0].Text="K - Kapacita [vozíkù+mezer]";
	mGrid->Cells[16][0].Text="P - Pozice [vozíkù]";
	...
	mGrid->SetCells(mGrid->Cells[0][0],12,0,ColCount-1,0);//pro první øádek
	mGrid->SetCells(mGrid->Cells[0][0],1,0,2,0);//pro první øádek
...
//	mGrid->SetCells(mGrid->Cells[ColCount-1][1],ColCount-1,2,ColCount-1,RowCount-1);
	...
	Canvas->Font=mGrid->Cells[4][0].Font;	//nejdelší použitý text
	...
	mGrid->Columns[1].Width=40;mGrid->Columns[2].Width=mGrid->Columns[3].Width=mGrid->Columns[4].Width=mGrid->Columns[5].Width=mGrid->Columns[6].Width=mGrid->Columns[7].Width=mGrid->Columns[8].Width=mGrid->Columns[9].Width=mGrid->Columns[10].Width=mGrid->Columns[11].Width=35;//ostatní následující sloupce zatím default šíøka
