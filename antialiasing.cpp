...
Graphics::TBitmap *Cantialising::antialiasing(Graphics::TBitmap *bmp_in,bool transparent)
{
	//zoom
	...
	if(transparent){bmp_out->Transparent=true;bmp_out->TransparentColor=clWhite;bmp_out->TransparentMode = tmAuto;}//již se nevyužívá, ale nechávám pro pøípadné další použití, je nutné ale mít bmp_out->PixelFormat=pf24bit;
...
//---------------------------------------------------------------------------
//uloží bitmapu pøesampovanou pomocí algoritmu antialiasingu
void Cantialising::antialiasing(Graphics::TBitmap *bmp_in,Graphics::TBitmap *bmp_out)
{
	//zoom
	short Z=3;//*3 vyplývá z logiky algoritmu antialiasingu
	...
	//bmp_out->Transparent=true;bmp_out->TransparentColor=clWhite;bmp_out->TransparentMode = tmAuto;//již se nevyužívá, ale nechávám pro pøípadné další použití, je nutné ale mít bmp_out->PixelFormat=pf24bit;
	...
	for(int Y=0;Y<=bmp_out->Height-1;Y++)// pro všechny øadky
	{
		...
		for(int X=0;X<=bmp_out->Width-1;X++)
		{
			//pokud bych chtìl vstupní bmp naèítát nad aktulní semplovanou bitmapou if(RD[X].rgbtRed==255 && RD[X].rgbtGreen==255 && RD[X].rgbtBlue==255) šlo by nastavovat parametrem transparent paremetrem metody
			{
				int CX=X*Z;// zpracuju vzorky z 3 x 3 pixelu
				// inicializace vysledne barvy
				RD[X].rgbtRed=(R1[CX].rgbtRed+R2[CX].rgbtRed+R3[CX].rgbtRed+R1[CX+1].rgbtRed+R2[CX+1].rgbtRed+R3[CX+1].rgbtRed+R1[CX+2].rgbtRed+R2[CX+2].rgbtRed+R3[CX+2].rgbtRed)/9;
...
//---------------------------------------------------------------------------
//již se nevyužívá, ale zanechávám pro pøíklad použití dvou bmp
////vrátí bitmapu pøesamplovanou pomocí algoritmu antialiasingu
