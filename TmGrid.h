...
{
 public:
	enum Ttype{TEXT,EDIT,BUTTON,COMBO,CHECK,RADIO,/*IMAGE,*/DRAW};
	...
	void Show(TCanvas *Canvas,TForm *Form);//zajistí vykreslení celé tabulky
	void Refresh(TCanvas *Canvas,TForm *Form);//zajistí pøekreslení celé tabulky
	...
	//unsigned short ColWidth,RowHeight;//výška a šíøka øádku
 ...
 private:
	...
	void DrawGrid(TCanvas *Canvas,TForm *Form,double Zoom);//zajistí vykreslení celé tabulky
	void SetBorder(TCanvas *C,TmBorder Border);//nastaví grafické pero na požadované parametry
	void SetComponents(TForm *Form,TCanvas *Canv,TRect R,unsigned long X,unsigned long Y,TCells Cell);//nastaví danou buòku dle typu
	void SetEdit(TForm *Form,TCanvas *Canv,TRect R,unsigned long X,unsigned long Y,TCells Cell);//nastaví danou buòku na edit, pomocná metoda výše uvedené
