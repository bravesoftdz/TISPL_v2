...
class TmGrid
{
 public:
	...
	struct TCells//datový typ používaný pro buňky
	{
		...
		bool isEmtyConditionalFormattingState;//zapínání či vypínání podmíněného formátování pozadí buňky
	...
	void AddColumn(bool copyFromPreviousColumn=true,bool invalidate=true);//přidá sloupec za poslední sloupec, pokud copyPropertiesFromPreviousRow je na true, zkopiruje kompomenty a vlasnosti z předchozího sloupce, pokud je invalidate na true, automaticky po přidání překreslí tabulku, někdy pokud nechci problikávat tabulku lépe nastavit na false a zavolat formpaint přímo za voláním metody AddColumn přimo v užitém formuláři
	void InsertColumn(unsigned long Column,bool copyPropertiesFromPreviousColumn=true,bool invalidate=true);//přídá sloupec za sloupec uvedený dle parametru Column, pokud copyropertiesFromPreviouscolumn je na true, zkopiruje kompomenty a vlasnosti z předchozího sloupce, pokud je invalidate na true, automaticky po přidání překreslí tabulku, někdy pokud nechci problikávat tabulku lépe nastavit na false a zavolat formpaint přímo za voláním metody InsertColumn přimo v užitém formuláři. Pozor, po přidání sloupce za zmergovaní oblast, nevkládá komponenty z mergované buňky i přes copyPropertiesFromPreviousColumn, více vysvětlení v mergecells sekce setcells
...
//	void HighlightNumeric(unsigned long Col,unsigned long Row,TColor Color=clRed,unsigned short Width=1);//zajistí trvalé (jedná se spíše o nastavení) zvýraznění dané komponenty
 ...
 private:
  ...
  void SetComboEdit(TRect R,unsigned long X,unsigned long Y,TCells &Cell);//nastaví danou buňku na combo, pomocná metoda objednu výše uvedené
	...
	unsigned long getTag(unsigned long Col,unsigned long Row,unsigned long ColCount);//vratí ID tag komponenty,absolutní pořadí v paměti
